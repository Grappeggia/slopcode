...
export default {
  up(tx) {
    return Effect.gen(function* () {
      ...
      yield* tx.run(`CREATE UNIQUE INDEX \`memory_global_scope_hash_idx\` ON \`memory\` (\`scope\`,\`hash\`) WHERE "memory"."project_id" IS NULL;`)
      yield* tx.run(`CREATE UNIQUE INDEX \`memory_project_scope_hash_idx\` ON \`memory\` (\`scope\`,\`project_id\`,\`hash\`) WHERE "memory"."project_id" IS NOT NULL;`)
