export * as SlopCode from "./slopcode"
...
/** Intentional public native API for Effect applications embedding SlopCode. */
export class Service extends Context.Service<Service, Interface>()("@slopcode/public/SlopCode") {}
...
>()("@slopcode/public/SlopCode/SessionModelValidation") {}
...
const ConfigNames = ["config.json", "slopcode.json", "slopcode.jsonc"] as const
...
type ConfigModel = NonNullable<NonNullable<Config.Info["providers"]>[string]["models"]>[string]
...
async function localModel(input: Session.SwitchModelInput & { readonly location: Session.Info["location"] }) {
  let model: ConfigModel | undefined
  for (const directory of directories(input.location.directory)) {
    for (const base of [directory, path.join(directory, ".slopcode")]) {
      for (const name of ConfigNames) {
        const text = await Bun.file(path.join(base, name))
          .text()
          .catch(() => undefined)
        if (!text) continue
        ...
        const info = Option.getOrUndefined(DecodeConfig(parsed))
        const next = info?.providers?.[input.model.providerID]?.models?.[input.model.id]
        if (next) model = { ...model, ...next, variants: next.variants ?? model?.variants }
      }
    }
  }
  return model
...
const validateLocalModel = Effect.fn("SlopCode.sessions.validateLocalModel")(function* (
  input: Session.SwitchModelInput & { readonly location: Session.Info["location"] },
) {
  const model = yield* Effect.promise(() => localModel(input))
...
const SessionModelValidationLayer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    return SessionModelValidation.of({
      validate: Effect.fn("SlopCode.sessions.validateModel")(function* (input) {
        yield* Effect.gen(function* () {
          ...
          Effect.provide(locations.get(input.location)),
          Effect.catchCauseIf(
            (cause) => Cause.hasInterruptsOnly(cause),
            () => validateLocalModel(input),
          ),
        )
      }),
    })
...
// TODO: Add SlopCode.create(...) as the Promise facade over the same native API semantics.
