...
export class Service extends Context.Service<Service, Interface>()("@slopcode/Event") {}
      ...
      function publish<D extends Definition>(definition: D, data: Data<D>, options?: PublishOptions) {
        return Effect.gen(function* () {
          ...
          const location = inputLocation
            ? Location.Ref.make({ directory: inputLocation.directory, workspaceID: inputLocation.workspaceID })
            : undefined
          const payloadData = definition.sync ? normalizeData(data) : data
...
function normalizeData<D extends Definition>(data: Data<D>): Data<D> {
  if (typeof data !== "object" || data === null || !("location" in data)) return data
  const value = (data as Record<string, unknown>).location
  if (value instanceof Location.Ref) return data
  if (typeof value !== "object" || value === null || !("directory" in value)) return data
  const ref = value as Pick<Location.Ref, "directory" | "workspaceID">
  return {
