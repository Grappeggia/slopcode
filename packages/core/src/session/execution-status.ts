...
export class TransitionRejected extends Schema.TaggedErrorClass<TransitionRejected>()(
  "SessionExecutionStatus.TransitionRejected",
  {
    sessionID: SessionSchema.ID,
...
export class DurableTerminalError extends Schema.TaggedErrorClass<DurableTerminalError>()(
  "SessionExecutionStatus.Terminal",
  {
    sessionID: SessionSchema.ID,
    code: TerminalCode,
    message: Schema.String,
...
export interface Interface {
  ...
  readonly list: (input?: {
    readonly owner?: SessionRuntime.Owner
    readonly nonIdle?: boolean
  }) => Effect.Effect<ReadonlyArray<{ readonly sessionID: SessionSchema.ID; readonly status: Info }>>
  ...
  readonly dispatch: (
    input: ActiveInput & {
      ...
      readonly fingerprint: string
      readonly now?: number
      readonly recovery?: "retry-provider" | "continue-provider" | "interrupt"
    },
  ) => Effect.Effect<{ readonly event?: EventV2.Payload; readonly claimed: boolean }>
  readonly complete: (
    input: ActiveInput & {
      readonly requestAttempt: number
      readonly providerAttempt: number
  ...
  readonly retry: (
    input: ActiveInput & {
      ...
      readonly code: SessionEvent.Execution.RetryCode
      readonly action: SessionEvent.Execution.RetryAction
      readonly message: string
      readonly fingerprint: string
      readonly recovery?: "retry-provider" | "interrupt"
    },
  ) => Effect.Effect<EventV2.Payload>
  readonly succeed: (input: Fence & IdentityInput & { readonly release?: boolean }) => Effect.Effect<EventV2.Payload>
  readonly interrupt: (
    input: ActiveInput & { readonly code: TerminalCode; readonly message: string; readonly resultingEpoch: number },
  ) => Effect.Effect<EventV2.Payload>
  readonly fail: (
    input: ActiveInput & { readonly code: TerminalCode; readonly message: string; readonly resultingEpoch: number },
  ) => Effect.Effect<EventV2.Payload>
...
const hash = (...parts: ReadonlyArray<string | number | undefined>) =>
  createHash("sha256")
    .update(parts.map((part) => `${String(part).length}:${String(part)}`).join("|"))
    .digest("hex")
export const eventID = (input: {
  ...
  EventV2.ID.make(
    `evt_${hash("session-execution-v1", input.sessionID, input.activityID, input.rootID, input.epoch, input.requestAttempt, input.structuredAttempt, input.providerAttempt, input.kind)}`,
  )
...
export const make = Effect.gen(function* () {
  ...
  const get = Effect.fn("SessionExecutionStatus.get")(function* (sessionID: SessionSchema.ID) {
    ...
    const row = yield* db
      .select()
      .from(SessionExecutionStatusTable)
      .where(eq(SessionExecutionStatusTable.session_id, sessionID))
      .get()
      .pipe(Effect.orDie)
    return row ? decode(row.data) : ({ type: "idle" } as const)
  })
  const guard = (
    input: Fence & IdentityInput,
    predecessors: ReadonlyArray<Info["type"]>,
    validate?: (status: Info) => boolean,
  ) =>
    Effect.gen(function* () {
      const runtime = yield* db
        .select()
        .from(SessionTable)
      ...
      if (
        !runtime ||
        runtime.runtime !== "v2" ||
        runtime.runtime_state !== input.runtimeState ||
        runtime.runtime_epoch !== input.epoch
      )
        return yield* new TransitionRejected({ sessionID: input.sessionID, message: "Session runtime fence changed" })
      const row = yield* db
        .select()
        .from(SessionExecutionStatusTable)
        .where(eq(SessionExecutionStatusTable.session_id, input.sessionID))
        .get()
        .pipe(Effect.orDie)
      ...
      if (!predecessors.includes(type) || (type !== "idle" && !replacement && !same(row, input)))
        return yield* new TransitionRejected({
          ...
          message: "Illegal execution status predecessor",
        })
  ...
  const publishEvent = events.publish as unknown as (
    definition: EventV2.Definition,
    data: unknown,
  ...
  const publish = (
    ...
    input: ActiveInput | (Fence & IdentityInput),
    predecessors: ReadonlyArray<Info["type"]>,
    ...
    Effect.gen(function* () {
      const id = eventID({
        ...
        requestAttempt: "requestAttempt" in input ? input.requestAttempt : undefined,
        providerAttempt: "providerAttempt" in input ? input.providerAttempt : undefined,
        structuredAttempt: "structuredAttempt" in input ? input.structuredAttempt : undefined,
      })
      return yield* publishEvent(
        definition,
        { ...data, timestamp: yield* timestamp(id) },
        {
          id,
          idempotent: true,
          equivalent,
  ...
  const release = (input: Fence & IdentityInput, resultingEpoch: number, state: "ready" = "ready") =>
    db
      .update(SessionTable)
      .set({ runtime_state: state, runtime_epoch: resultingEpoch })
      .where(
        and(
          eq(SessionTable.id, input.sessionID),
          eq(SessionTable.runtime, "v2"),
          eq(SessionTable.runtime_state, input.runtimeState),
          eq(SessionTable.runtime_epoch, input.epoch),
        ),
      )
      ...
      .pipe(
        Effect.orDie,
        Effect.flatMap((row) => (row ? Effect.void : Effect.die("Session runtime release fence changed"))),
      )
  ...
  return Service.of({
    ...
    list: Effect.fn("SessionExecutionStatus.list")(function* (input = {}) {
      const rows = yield* db
        .select({ sessionID: SessionTable.id, owner: SessionTable.runtime, data: SessionExecutionStatusTable.data })
        .from(SessionTable)
        .leftJoin(SessionExecutionStatusTable, eq(SessionExecutionStatusTable.session_id, SessionTable.id))
        .where(input.owner ? eq(SessionTable.runtime, input.owner) : undefined)
        .orderBy(asc(SessionTable.id))
        .all()
        .pipe(Effect.orDie)
      return rows.flatMap((row) => {
        const status = row.data ? decode(row.data) : ({ type: "idle" } as const)
        return input.nonIdle && status.type === "idle"
          ? []
          : [{ sessionID: SessionSchema.ID.make(row.sessionID), status }]
      })
    }),
    start: (input) =>
      publish(SessionEvent.Execution.Started, eventData(input), input, ["idle", "interrupted", "terminal-failure"]),
    dispatch: (input) =>
      Effect.gen(function* () {
        let claimed = false
        const now = input.now ?? (yield* Clock.currentTimeMillis)
        const validate = (current: Info) =>
          current.type === "retrying"
            ? current.recovery === "retry-provider" &&
              current.nextAt <= now &&
              current.requestAttempt === input.requestAttempt &&
              current.providerAttempt === input.providerAttempt &&
            ...
            : current.type === "busy" &&
              (current.requestAttempt === undefined
                ...
                : current.recovery === "continue-provider" &&
                  input.requestAttempt === current.requestAttempt + 1 &&
    ...
    complete: (input) =>
      publish(
        SessionEvent.Execution.ProviderCompleted,
        eventData(input),
        input,
        ["busy"],
        undefined,
    ...
    continue: (input) =>
      publish(
        SessionEvent.Execution.ContinuationReady,
        { ...eventData(input), recovery: "continue-provider" },
        input,
        ["busy"],
        undefined,
        (current) =>
    ...
    retry: (input) =>
      publish(
        SessionEvent.Execution.RetryScheduled,
        {
          ...eventData(input),
          message: SessionProviderRetry.sanitize(input.message),
          recovery: input.recovery ?? "retry-provider",
        },
        input,
        ["busy"],
        undefined,
        (current) =>
          ...
          input.providerAttempt === current.providerAttempt + 1 &&
          current.fingerprint === input.fingerprint,
      ),
    succeed: (input) =>
      publish(
        ...
        input,
        ["busy"],
        input.release === false ? undefined : () => release(input, input.epoch + 1),
      ),
    interrupt: (input) =>
      publish(
        SessionEvent.Execution.Interrupted,
        { ...eventData(input), message: SessionProviderRetry.sanitize(input.message) },
        input,
        ["busy", "retrying"],
        () => release(input, input.resultingEpoch),
      ),
    fail: (input) =>
      publish(
    ...
    replace: (input) =>
      Effect.gen(function* () {
        const id = eventID({
          ...input,
          kind: `${SessionEvent.Execution.Interrupted.type}:runtime-replaced`,
          requestAttempt: input.requestAttempt,
          providerAttempt: input.providerAttempt,
          structuredAttempt: input.structuredAttempt,
        })
        const data = {
          ...eventData(input),
        ...
        return yield* publishEvent(SessionEvent.Execution.Interrupted, data, {
          ...
          guard: () =>
            Effect.gen(function* () {
              ...
              if (
                !runtime ||
                runtime.runtime !== input.resultingOwner ||
                runtime.runtime_state !== input.resultingState ||
                runtime.runtime_epoch !== input.resultingEpoch
              )
                return yield* new TransitionRejected({
                  sessionID: input.sessionID,
                  message: "Replacement runtime fence changed",
                })
              if (
                !row ||
                row.epoch !== input.epoch ||
                !same(row, input) ||
                (decode(row.data).type !== "busy" && decode(row.data).type !== "retrying")
              )
                return yield* new TransitionRejected({
                  sessionID: input.sessionID,
                  message: "Replacement execution status changed",
                })
            }).pipe(Effect.orDie),
        })
      }),
...
export const project = (events: EventV2.Interface, db: Database.Interface["db"]) => {
  const active = (
    ...
    return db
      ...
      .onConflictDoUpdate({
        target: SessionExecutionStatusTable.session_id,
        set: {
          activity_id: event.data.activityID,
          root_id: event.data.rootID,
          owner: event.data.owner,
  ...
  const retry = (event: SessionEvent.Execution.RetryScheduled) => {
    ...
    return db
      ...
      .set({ epoch: event.data.epoch, seq: event.seq, data: encode(status) })
      .where(eq(SessionExecutionStatusTable.session_id, event.data.sessionID))
      .run()
      .pipe(Effect.orDie)
  }
  const terminal =
    (type: "interrupted" | "terminal-failure") =>
    (event: SessionEvent.Execution.Interrupted | SessionEvent.Execution.Failed) => {
      if (event.seq === undefined) return Effect.die("Execution event is missing aggregate sequence")
      const status = { type, ...event.data, epoch: event.data.resultingEpoch, seq: event.seq }
      return db
        .update(SessionExecutionStatusTable)
        .set({ epoch: event.data.resultingEpoch, seq: event.seq, data: encode(status) })
        .where(eq(SessionExecutionStatusTable.session_id, event.data.sessionID))
        .run()
        .pipe(Effect.orDie)
    }
  return Effect.all(
    [
      ...
      events.project(SessionEvent.Execution.Succeeded, (event) =>
        db
          .delete(SessionExecutionStatusTable)
          .where(eq(SessionExecutionStatusTable.session_id, event.data.sessionID))
          .run()
          .pipe(Effect.orDie),
      ),
