...
const status = (reason: LLMErrorReason) => {
  ...
  return ("http" in reason ? reason.http?.response?.status : undefined) ?? (reason._tag === "UnknownProvider" ? reason.status : undefined)
}
...
export const classify = (failure: unknown): Notice | undefined => {
  if (failure instanceof LLMError) {
    ...
    const code = failure.reason._tag === "RateLimit"
      ? "rate-limit"
  ...
  if (!Schema.is(ProviderErrorEvent)(failure) || failure.retryable !== true || failure.classification === "context-overflow") return
  return { code: "explicit", action: "retry-provider", message: sanitize(failure.message) }
}

export const notice = (failure: unknown) => classify(failure) ?? {
...
export const hints = (failure: unknown): Hint => {
  ...
  return {
    ...
    ...(headers === undefined ? {} : { headers: Object.fromEntries(Object.entries(headers).flatMap(([key, value]) => typeof value === "string" ? [[key.toLowerCase(), value]] : [])) }),
  }
}
