...
const secret =
  /^(?:authorization|proxy-authorization|(?:x-)?api[-_]?key|token|secret|credential|cookie|set-cookie|auth|password)$/i
...
const load = async (data: string) => {
  ...
  if (!directory.isDirectory() || directory.isSymbolicLink())
    throw new Error("Request fingerprint key directory is unsafe")
  ...
  if (!existing) {
    const temp = path.join(
      target.directory,
      `.request-fingerprint.${process.pid}.${randomBytes(16).toString("hex")}.tmp`,
    )
...
export const fromKey = (key: Uint8Array): Interface => {
  ...
  const credential = (names: ReadonlyArray<string>, value: unknown) =>
  ...
  const canonical = (value: unknown, names: ReadonlyArray<string> = []): unknown => {
    ...
    return Object.fromEntries(
      Object.entries(value)
        ...
        .map(([name, item]) => [
          name,
          secret.test(name) ? { credential: credential([...names, name], item) } : canonical(item, [...names, name]),
        ]),
    )
  }
  return {
...
export const make = (input: { readonly data: string }) =>
  Effect.tryPromise({
    try: () => load(input.data).then(fromKey),
      ...
      new KeyError({
        message: error instanceof Error ? error.message : "Request fingerprint key initialization failed",
      }),
  })
...
export const layer = Layer.effect(
  Service,
  Global.Service.pipe(
    Effect.flatMap((global) => make(global)),
    Effect.orDie,
  ),
)
