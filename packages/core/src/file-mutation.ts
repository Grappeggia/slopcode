...
export class UnsupportedPlatformError extends Schema.TaggedErrorClass<UnsupportedPlatformError>()(
  "FileMutation.UnsupportedPlatformError",
  {
    platform: Schema.String,
  },
) {}

export class StageUnavailableError extends Schema.TaggedErrorClass<StageUnavailableError>()(
  "FileMutation.StageUnavailableError",
  {
    path: Schema.String,
  },
) {}
...
export interface Interface {
  ...
  ) => Effect.Effect<string, Error | unknown>
...
const revisionIdentity = (value: Revision) => `${value.dev}:${value.ino}:${value.size}:${value.mtime}:${value.ctime}`
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const supported = (target: Target) =>
      platform.capabilities.mutation
        ? Effect.void
    ...
    const close = (handle: FileHandle | undefined) =>
      handle ? Effect.promise(() => handle.close()).pipe(Effect.ignore) : Effect.void

    const parent = (target: Target, create: boolean) =>
      Effect.acquireRelease(
        Effect.gen(function* () {
          yield* supported(target)
          const parts = path.dirname(target.canonical).split(path.sep).filter(Boolean)
          const opened: FileHandle[] = []
          const root = yield* safe(target, () =>
            fs.open("/", constants.O_RDONLY | constants.O_DIRECTORY | constants.O_NOFOLLOW),
          )
          opened.push(root)
    ...
    const open = (target: Target, createDirs: boolean) =>
      parent(target, createDirs).pipe(
          ...
          Effect.acquireRelease(
            Effect.tryPromise({
              try: () =>
                ...
                code(error) === "ENOENT" ? new Missing() : new TargetChangedError({ path: target.canonical }),
            }),
            close,
          ),
        ),
    ...
    const createHandle = (target: Target, content: Uint8Array) =>
      parent(target, true).pipe(
        Effect.flatMap((directory) =>
          Effect.acquireRelease(
            Effect.tryPromise({
              try: () =>
      ...
      open(target, createDirs).pipe(
        Effect.flatMap((handle) =>
          Effect.gen(function* () {
            ...
            if (wanted && !sameRevision(revision(stat), wanted))
              return yield* new StaleContentError({ path: target.canonical })
            ...
            return result(
              ...
              "changed",
              content,
              revision(yield* safe(target, () => handle.stat({ bigint: true }))),
            )
          }),
        ),
      )
    ...
    const write = Effect.fn("FileMutation.write")((input: WriteInput) =>
      supported(input.target).pipe(
        Effect.andThen(
          withLock(
            input.target,
            writeOpen(input.target, bytes(input.content), undefined, undefined, true).pipe(
              Effect.catchIf(
                (error) => error instanceof Missing,
                () => createHandle(input.target, bytes(input.content)),
              ),
            ),
          ),
        ),
          ...
          Effect.flatMap((handle) =>
            Effect.gen(function* () {
              ...
              return result(
                ...
                "changed",
                content,
                revision(yield* safe(input.target, () => handle.stat({ bigint: true }))),
              )
            }),
          ),
          Effect.catchIf(
    ...
    const writeIfUnchanged = Effect.fn("FileMutation.writeIfUnchanged")(function* (input: ConditionalWriteInput) {
      ...
      return yield* withLock(
        input.target,
        writeOpen(input.target, bytes(input.content), input.expected).pipe(
          Effect.catchIf(
            (error) => error instanceof Missing,
            () => Effect.fail(new TargetChangedError({ path: input.target.canonical })),
          ),
        ),
      )
    })

    const commit = Effect.fn("FileMutation.commit")(
      (
        input: ConditionalWriteInput & {
          readonly revision: unknown
          readonly guard?: Effect.Effect<void, unknown>
        ...
        withLock(
          input.target,
          input.revision && typeof input.revision === "object"
            ? writeOpen(
                input.target,
                bytes(input.content),
                input.expected,
        ...
        Effect.flatMap((directory) =>
          Effect.gen(function* () {
            ...
            const handle = yield* Effect.tryPromise({
              try: () =>
                fs.open(child, constants.O_RDWR | constants.O_CREAT | constants.O_EXCL | constants.O_NOFOLLOW, 0o600),
        ...
        Effect.andThen(
          withLock(
            ...
            parent(input.target, false).pipe(
              Effect.flatMap((directory) =>
                Effect.acquireRelease(
                  Effect.tryPromise({
                    try: () =>
                      fs.open(
                        platform.path(directory.fd, path.basename(input.target.canonical)),
                  ...
                  Effect.flatMap((handle) => {
                    ...
                    return Effect.gen(function* () {
                      ...
                      const named = yield* safe(input.target, () =>
                        fs.lstat(platform.path(directory.fd, path.basename(input.target.canonical)), { bigint: true }),
                      )
                      if (opened.dev !== named.dev || opened.ino !== named.ino)
                        return yield* new TargetChangedError({ path: input.target.canonical })
                      if (!platform.exchange || !platform.move || !platform.unlink)
                        return yield* new UnsupportedPlatformError({ platform: platform.name })
                      ...
                      const placeholder = yield* safe(input.target, () =>
                        fs.open(
                      ...
                      const state: { value: "placeholder" | "approved" | "public" | "done" | "conflict" } = {
                        value: "placeholder",
                      }
                      ...
                      return yield* Effect.gen(function* () {
                        ...
                        const quarantined = yield* safe(input.target, () =>
                          fs.lstat(platform.path(directory.fd, quarantine), { bigint: true }),
                        )
              ...
              Effect.catchIf(
                (error) => error instanceof Missing,
                () => Effect.succeed(removed(input.target, false)),
              ),
            ),
          ),
        ),
