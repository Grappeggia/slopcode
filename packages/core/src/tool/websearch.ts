...
export class ConfigService extends Context.Service<ConfigService, Config>()("@slopcode/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.SLOPCODE_WEBSEARCH_PROVIDER === "exa" || process.env.SLOPCODE_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.SLOPCODE_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("SLOPCODE_EXPERIMENTAL") || truthy("SLOPCODE_ENABLE_EXA") || truthy("SLOPCODE_EXPERIMENTAL_EXA"),
    enableParallel: truthy("SLOPCODE_ENABLE_PARALLEL") || truthy("SLOPCODE_EXPERIMENTAL_PARALLEL"),
