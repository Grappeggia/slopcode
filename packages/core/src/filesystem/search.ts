...
export const ripgrepLayer = Layer.effect(Service, makeRipgrep.pipe(Effect.map(Service.of)))
...
export const fffLayer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    if (!result.ok) return yield* makeRipgrep.pipe(Effect.map(Service.of))
...
export const defaultLayer = Layer.unwrap(
  Effect.sync(() => (Flag.SLOPCODE_DISABLE_FFF || !Fff.available() ? ripgrepLayer : fffLayer)),
)
