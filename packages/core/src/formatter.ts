...
export function resolve(entries: readonly Config.Entry[]): Entry[] {
  ...
  const merged = builtins.flatMap((item) => {
    ...
    if (
      ...
      !command(value) ||
      value?.disabled === true ||
      (pythonDisabled && (item.name === "ruff" || item.name === "uv"))
    )
      return []
    return [
      {
  ...
  return [
    ...
    ...order.flatMap((name) => {
      ...
      if (
        ...
        value.command.some((item) => item.length === 0)
      )
        return []
      if (!value.extensions?.length || value.extensions.some((item) => item.length === 0)) return []
      return [
        { name, command: value.command, environment: value.environment, extensions: value.extensions, builtin: false },
      ]
    }),
  ]
}
  ...
  Effect.gen(function* () {
    ...
    const manifest = Effect.fnUntraced(function* (name: string, filename: "package.json" | "composer.json") {
      for (const file of yield* fs.findUp(filename, location.directory, location.project.directory)) {
        ...
        const groups =
          filename === "package.json" ? [json.dependencies, json.devDependencies] : [json.require, json["require-dev"]]
        if (groups.some((group) => typeof group === "object" && group !== null && Object.hasOwn(group, name)))
          return true
      }
      return false
    })
    const npmWhich = (pkg: string) =>
      npm ? npm.which(pkg).pipe(Effect.map(Option.getOrUndefined)) : Effect.succeed(undefined)
    const probe = (argv: readonly string[]) =>
      app
        .run(ChildProcess.make(argv[0]!, argv.slice(1), { cwd: location.directory, stdin: "ignore" }), {
          timeout: DISCOVERY_TIMEOUT,
          maxOutputBytes: OUTPUT_LIMIT,
          maxErrorBytes: OUTPUT_LIMIT,
        })
        .pipe(Effect.option)

    const builtin = (item: Entry): Effect.Effect<readonly string[] | undefined, FSUtil.Error> =>
      Effect.gen(function* () {
        ...
        switch (item.name) {
          ...
          case "biome": {
            const found =
              (yield* fs.findUp("biome.json", location.directory, location.project.directory)).length > 0 ||
    ...
    const list = Effect.fn("Formatter.list")(function* () {
      return yield* Effect.forEach(catalog, inspect)
    })
    const format = Effect.fn("Formatter.format")(function* (target: { readonly canonical: string }) {
      ...
      for (const found of available) {
        if (!found.argv) {
          outcomes.push({ name: found.item.name, code: "unavailable" })
          continue
        }
        ...
        if (
          ...
          !Object.entries(environment).every(
            ([key, value]) => key.length > 0 && !key.includes("=") && !key.includes("\0") && !value.includes("\0"),
          )
        ) {
