...
export const hostLayer = Layer.effect(
  HostService,
  Effect.acquireRelease(Effect.promise(makeHost), (host) => Effect.promise(() => host.close())).pipe(Effect.map(HostService.of)),
)
export const locationLayer = Layer.effect(
  Service,
  Effect.gen(function* () {
    const host = yield* HostService
    return yield* Effect.acquireRelease(Effect.promise(() => host.lease()), (lease) => Effect.promise(() => lease.close())).pipe(
...
export async function makeHost(): Promise<Host> {
  ...
  const lease = async (): Promise<Interface> => {
    ...
    const register: Interface["register"] = async (input) => {
      ...
      if (states.has(stateKey) || claims.has(stateKey)) throw new Error("MCP OAuth callback registration is unavailable")
      claims.add(stateKey)
      const current = servers.get(serverKey)
      const pending = current ? Promise.resolve(current) : (starting.get(serverKey) ?? start(url.hostname, Number(url.port), serverKey))
      if (!current && !starting.has(serverKey)) starting.set(serverKey, pending)
      const server = await pending.finally(() => starting.delete(serverKey)).catch((error) => {
