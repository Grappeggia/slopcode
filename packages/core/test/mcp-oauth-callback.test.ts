...
describe("MCP OAuth callback", () => {
  ...
  it.live("shares listeners across callback services and unregisters only after durable acceptance", () =>
    Effect.acquireRelease(
      ...
      (value) =>
        Effect.promise(() =>
          Promise.all(value.callbacks.map((callback) => callback.close())).then(() => value.host.close()),
        ),
    ).pipe(
      Effect.flatMap(({ callbacks: [first, second] }) =>
        Effect.gen(function* () {
          ...
          expect(
            (yield* Effect.promise(() => fetch(`http://127.0.0.1:${port}/one?state=state-one&code=code`))).status,
          ).toBe(400)
          expect(
            (yield* Effect.promise(() => fetch(`http://127.0.0.1:${port}/one?state=state-one&code=code`))).status,
          ).toBe(200)
          ...
          expect(
            (yield* Effect.promise(() => fetch(`http://127.0.0.1:${port}/two?state=state-two&error=denied`))).status,
          ).toBe(400)
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((callbacks) =>
        Effect.gen(function* () {
          ...
          expect(
            (yield* Effect.promise(() =>
              fetch(`http://127.0.0.1:${port}/callback?state=duplicates&state=duplicates&code=code`),
            )).status,
          ).toBe(400)
          expect(
            (yield* Effect.promise(() => fetch(`http://127.0.0.1:${port}/callback?state=duplicates&code=one&code=two`)))
              .status,
          ).toBe(400)
          ...
          ipv6.stop(true)
          const six = yield* Effect.promise(() =>
            callbacks.register({
          ...
          expect(
            (yield* Effect.promise(() => fetch(`http://[::1]:${ipv6Port}/callback?state=ipv6-state&code=code`))).status,
          ).toBe(200)
