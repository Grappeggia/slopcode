...
const flagsLayer = ConfigProvider.layer(
  ConfigProvider.fromUnknown({
    SLOPCODE_EXPERIMENTAL_FILEWATCHER: "true",
    SLOPCODE_EXPERIMENTAL_DISABLE_FILEWATCHER: "false",
  }),
)
...
function withTmp<A, E, R>(
  ...
  return Effect.acquireRelease(
    Effect.promise(async () => {
      ...
      await $`git config user.email test@slopcode.test`.cwd(tmp.path).quiet()
