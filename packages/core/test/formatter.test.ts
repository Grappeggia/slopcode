...
const withFormatter = <A, E, R>(directory: string, entries: Config.Entry[], effect: Effect.Effect<A, E, R>) =>
  effect.pipe(
    Effect.provide(
...
describe("Formatter", () => {
  it.effect("implements omitted, boolean, reset, re-enable, and malformed config semantics", () =>
    Effect.sync(() => {
      ...
      expect(
        Formatter.resolve([
          document({ custom: { command: ["run"], extensions: [".x"] } }),
          document({ custom: { disabled: true } }),
          document({ custom: { disabled: false } }),
        ]).at(-1)?.name,
      ).toBe("custom")
      expect(
        Formatter.resolve([document({ custom: { command: [], extensions: [".x"] } })]).some(
          (item) => item.name === "custom",
        ),
      ).toBe(false)
      expect(
        Formatter.resolve([document({ custom: { command: ["run"], extensions: [] } })]).some(
          (item) => item.name === "custom",
        ),
      ).toBe(false)
      expect(Formatter.resolve([document({ gofmt: { command: [] } })]).some((item) => item.name === "gofmt")).toBe(
        false,
      )
      expect(
        Formatter.resolve([
          document({ custom: { command: ["run"], extensions: [".x"], environment: { "BAD=KEY": "value" } } }),
        ]).some((item) => item.name === "custom"),
      ).toBe(false)
    }),
  )
  ...
  it.effect("preserves the complete built-in catalog and compatibility extensions", () =>
    Effect.sync(() => {
      expect(Formatter.resolve([document(true)]).map((item) => [item.name, item.extensions])).toEqual([
        ...
        [
          "clang-format",
          [".c", ".cc", ".cpp", ".cxx", ".c++", ".h", ".hh", ".hpp", ".hxx", ".h++", ".ino", ".C", ".H"],
        ],
        ["ktlint", [".kt", ".kts"]],
        ["ruff", [".py", ".pyi"]],
        ["air", [".R"]],
  ...
  it.effect("reads the oxfmt flag at runtime with experimental inheritance", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => ({
        experimental: process.env.SLOPCODE_EXPERIMENTAL,
        oxfmt: process.env.SLOPCODE_EXPERIMENTAL_OXFMT,
      })),
      () =>
        Effect.sync(() => {
            ...
            withFormatter(
              ...
              [
                document({
                  ...
                  every: {
                    command: [process.execPath, "-e", write, "$FILE", "before:$FILE:after", "$FILE"],
                    extensions: [".edge"],
                  },
              ...
              Effect.gen(function* () {
                ...
                expect(yield* formatter.format({ canonical: `${target}.UPPER` })).toEqual({
                  matched: false,
                  outcomes: [],
                })
      ...
      ({ tmp, bin }) => {
        ...
        return withFormatter(
          ...
          Effect.gen(function* () {
            ...
            yield* Effect.all([formatter.format({ canonical: first }), formatter.format({ canonical: second })], {
  ...
  it.live("drops cached discovery when the actual Location formatter scope reopens", () =>
    Effect.acquireUseRelease(
      Effect.promise(async () => {
        ...
        const second = path.join(tmp.path, "second")
        await Promise.all([fs.mkdir(first), fs.mkdir(second)])
        const make = (directory: string, value: string) =>
          fs.writeFile(
        ...
        return Effect.gen(function* () {
          ...
          for (const [index, [name, content]] of evidence.entries()) {
            ...
            yield* withFormatter(
              ...
              [document(true)],
              Effect.gen(function* () {
                const result = yield* (yield* Formatter.Service).format({
                  canonical: path.join(directory, "source.py"),
                })
        ...
        return Effect.gen(function* () {
          ...
          yield* withFormatter(
            ...
            Effect.gen(function* () {
              ...
              yield* Effect.promise(() => fs.writeFile(file, "source"))
              expect((yield* (yield* Formatter.Service).format({ canonical: file })).outcomes[0]).toMatchObject({
                name: "air",
                code: "formatted",
              })
