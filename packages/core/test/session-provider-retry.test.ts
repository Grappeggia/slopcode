...
describe("SessionProviderRetry", () => {
  ...
  test("uses exact bounded backoff and canonical hint precedence", () => {
    ...
    expect(SessionProviderRetry.delay(1, { headers: { "retry-after-ms": "2500", "retry-after": "9" } }, 1_000)).toBe(
      2_500,
    )
    expect(SessionProviderRetry.delay(1, { headers: { "retry-after": "Thu, 01 Jan 1970 00:00:06 GMT" } }, 1_000)).toBe(
      5_000,
    )
    expect(SessionProviderRetry.delay(1, { retryAfterMs: Number.POSITIVE_INFINITY }, 1_000)).toBe(2_000)
    expect(SessionProviderRetry.delay(1, { headers: { "retry-after": "Thu, 01 Jan 1970 00:00:00 GMT" } }, 1_000)).toBe(
      2_000,
    )
  ...
  test("retries only explicit or canonical transient failures and honors deny classes", () => {
    expect(SessionProviderRetry.classify(error(new RateLimitReason({ message: "limited" })))).toMatchObject({
      code: "rate-limit",
    })
    expect(
      SessionProviderRetry.classify(error(new ProviderInternalReason({ message: "server", status: 500 }))),
    ).toMatchObject({ code: "server" })
    expect(SessionProviderRetry.classify(error(new TransportReason({ message: "retry me" })))).toBeUndefined()
    expect(
      SessionProviderRetry.classify(LLMEvent.providerError({ message: "explicit", retryable: true })),
    ).toMatchObject({ code: "explicit" })
    expect(
      SessionProviderRetry.classify(LLMEvent.providerError({ message: "rate limit 500", retryable: false })),
    ).toBeUndefined()
    expect(
      SessionProviderRetry.classify(error(new AuthenticationReason({ message: "no", kind: "invalid" }))),
    ).toBeUndefined()
    expect(
      SessionProviderRetry.classify(
        error(new InvalidRequestReason({ message: "too large", classification: "context-overflow" })),
      ),
    ).toBeUndefined()
    for (const reason of [
    ...
    expect(
      SessionProviderRetry.classify(error(new ProviderInternalReason({ message: "lower", status: 500 }))),
    ).toBeDefined()
    expect(
      SessionProviderRetry.classify(error(new ProviderInternalReason({ message: "upper", status: 599 }))),
    ).toBeDefined()
    expect(
      SessionProviderRetry.classify(error(new ProviderInternalReason({ message: "outside", status: 600 }))),
    ).toBeUndefined()
  })
  ...
  test("rejects malformed and negative hints while honoring present and future dates", () => {
    ...
    expect(SessionProviderRetry.delay(1, { headers: { "retry-after": "Thu, 01 Jan 1970 00:00:31 GMT" } }, 1_000)).toBe(
      30_000,
    )
  })
  ...
  test("sanitizes bounded durable retry metadata", () => {
    ...
    expect(JSON.stringify(forbidden)).not.toMatch(
      /secret-(?:message|header|response|body|request-id|metadata)|secret\.example/,
    )
    for (const value of [
      `Basic ${secret}`,
      `Bearer ${secret}`,
