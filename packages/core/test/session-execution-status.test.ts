...
describe("SessionExecutionStatus", () => {
  ...
  it.effect("retains bounded terminals until a new activity starts", () =>
    Effect.gen(function* () {
      ...
      yield* (yield* Database.Service).db
        .update(SessionTable)
        .set({ runtime_state: "draining", runtime_epoch: 3 })
      ...
      yield* (yield* Database.Service).db
        .update(SessionTable)
        .set({ runtime_state: "draining", runtime_epoch: 5 })
        .where(eq(SessionTable.id, sessionID))
        .run()
        .pipe(Effect.orDie)
      yield* service.start({
  ...
  it.effect("settles one root without releasing before a distinct root starts", () =>
    Effect.gen(function* () {
      ...
      expect(
        yield* db
          .select({ state: SessionTable.runtime_state, epoch: SessionTable.runtime_epoch })
          .from(SessionTable)
          .where(eq(SessionTable.id, sessionID))
          .get()
          .pipe(Effect.orDie),
      ).toEqual({ state: "draining", epoch: 1 })
    ...
    Effect.gen(function* () {
      ...
      const make = Effect.gen(function* () {
        const context = yield* Layer.buildWithScope(
          Layer.fresh(EventV2.layer.pipe(Layer.provide(Layer.succeed(Database.Service, database)))),
          scope,
        )
      ...
      const attempt = (candidate: SessionExecutionStatus.Interface) =>
        Effect.gen(function* () {
          arrivals++
          if (arrivals === 2) yield* Deferred.succeed(ready, undefined)
          yield* Deferred.await(gate)
          const claim = yield* candidate.dispatch({
            ...fence,
            ...activity,
            phase: "provider",
            requestAttempt: 1,
      ...
      const claims = yield* Effect.all([attempt(left), attempt(right)], { concurrency: "unbounded" }).pipe(
        Effect.forkChild,
      )
  ...
  it.effect("rolls back the status claim when deterministic dispatch event insertion fails", () =>
    Effect.gen(function* () {
      ...
      yield* events.beforeCommit((event) =>
        event.type === SessionEvent.Execution.ProviderDispatched.type
          ? Effect.die("forced dispatch insertion failure")
          : Effect.void,
      )
      ...
      expect(
        yield* db
          ...
          .where(
            eq(
              EventTable.id,
              SessionExecutionStatus.eventID({
                ...fence,
                ...activity,
                kind: SessionEvent.Execution.ProviderDispatched.type,
                requestAttempt: 1,
                providerAttempt: 2,
              }),
            ),
  ...
  it.effect("rebuilds the complete execution status projection from its event log", () =>
    Effect.gen(function* () {
      ...
      const recorded = yield* db
        .select()
        .from(EventTable)
        .where(inArray(EventTable.aggregate_id, replayIDs))
      ...
      yield* Effect.forEach(
        ...
        (kind) => {
          ...
          return events
            ...
            .pipe(
              Effect.andThen(
                db.delete(SessionExecutionStatusTable).where(eq(SessionExecutionStatusTable.session_id, id)).run(),
              ),
              Effect.orDie,
            )
        },
  ...
  it.effect("normalizes and caps messages at the status service boundary", () =>
    Effect.gen(function* () {
      ...
      expect(new TextEncoder().encode("message" in terminal ? terminal.message : "").byteLength).toBeLessThanOrEqual(
        512,
      )
    }),
  )
  ...
  it.effect("enforces the 512-byte UTF-8 message bound during synchronized publication", () =>
    Effect.gen(function* () {
      ...
      expect(
        (yield* events.publish(SessionEvent.Execution.Failed, { ...data, message: "😀".repeat(129) }).pipe(Effect.exit))
          ._tag,
      ).toBe("Failure")
    }),
  )
  ...
  it.effect("decodes continuation readiness through canonical durable and all-event unions", () =>
    Effect.gen(function* () {
      ...
      const event = yield* service.continue({
        ...fence,
        ...activity,
        phase: "tool",
        requestAttempt: 1,
