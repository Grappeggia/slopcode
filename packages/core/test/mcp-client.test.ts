        ...
        Effect.flatMap((fixture) =>
          Effect.scoped(
            Effect.gen(function* () {
              ...
              expect(
                (yield* Effect.promise(() => client.list(undefined, 5_000))).tools.map((tool) => tool.name),
              ).toContain("echo")
      ...
      Effect.acquireRelease(
        Effect.sync(() => {
          ...
          const server = Bun.serve({
            hostname: "127.0.0.1",
            port: 0,
            fetch: () => (requests++, new Response(null, { status: 401 })),
          })
        ...
        Effect.flatMap((fixture) =>
          Effect.scoped(
            Effect.gen(function* () {
              ...
              expect(yield* Effect.promise(() => Bun.file(path.join(tmp.path, "mcp-oauth/store.json")).exists())).toBe(
                false,
              )
            }),
          ),
        ...
        Effect.flatMap((fixture) =>
          Effect.scoped(
            Effect.gen(function* () {
              ...
              const context = yield* Layer.build(MCPClient.layerWith(observed))
              ...
              const error = yield* Context.get(context, MCPClient.Service)
                .connect({
                  name: target.name,
  ...
  Effect.acquireRelease(Effect.promise(tmpdir), (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]())).pipe(
    Effect.flatMap((tmp) =>
      Effect.acquireRelease(Effect.sync(refreshFixture), (fixture) =>
        Effect.sync(() => fixture.server.stop(true)),
      ).pipe(
        Effect.flatMap((fixture) =>
          Effect.scoped(
            Effect.gen(function* () {
      ...
      Effect.acquireRelease(Effect.sync(refreshFixture), (fixture) =>
        ...
        Effect.flatMap((fixture) =>
          Effect.gen(function* () {
            ...
            yield* seedRefresh(
              store,
              { directory: tmp.path, name: "refresh-process", endpoint },
              fixture.server.url.origin,
            )
            const worker = `${import.meta.dir}/fixture/mcp-oauth-refresh-worker.ts`
            const children = [0, 1].map(() =>
              Bun.spawn(["bun", worker, tmp.path, endpoint], {
                cwd: `${import.meta.dir}/..`,
                stdout: "pipe",
              }),
            )
            ...
            expect(fixture.refreshes()).toBe(1)
            expect(
              (yield* store.get({ directory: tmp.path, name: "refresh-process", endpoint })).tokens?.access_token,
            ).toBe("fresh")
          }),
        ),
      ),
...
it.live("bounds invalid grant malformed and interaction refresh failures", () =>
  Effect.acquireRelease(Effect.promise(tmpdir), (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]())).pipe(
    Effect.flatMap((tmp) =>
      Effect.forEach(
        ["invalid", "malformed", "interaction"] as const,
        (mode, index) =>
          Effect.acquireRelease(
            Effect.sync(() => refreshFailure(mode)),
            ...
            Effect.flatMap((server) =>
              Effect.scoped(
                Effect.gen(function* () {
                  ...
                  const target = {
                    directory: path.join(tmp.path, String(index)),
                    name: mode,
...
function refreshFailure(mode: "invalid" | "malformed" | "interaction") {
  return Bun.serve({
    ...
    fetch: (request) => {
      ...
      if (mode === "invalid")
        return Response.json({ error: "invalid_grant", error_description: "private" }, { status: 400 })
