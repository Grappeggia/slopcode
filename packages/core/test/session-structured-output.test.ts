...
describe("SessionFormat admission", () => {
    ...
    Effect.gen(function* () {
      const admitted = yield* SessionFormat.admit({
        ...
        schema: {
          ...
          $defs: {
            item: { type: "object", properties: { constructor: { type: "string" } }, required: ["constructor"] },
          },
          type: "array",
          items: { $ref: "#/$defs/item" },
        },
  ...
  it.effect("clones safely without invoking accessors or prototype setters", () =>
    Effect.gen(function* () {
      ...
      cyclic.self = cyclic
      expect(
        Exit.isFailure(yield* SessionFormat.admit({ type: "json_schema", schema: cyclic }).pipe(Effect.exit)),
      ).toBe(true)
  ...
  it.effect("applies byte and depth limits to the schema rather than its format wrapper", () =>
    Effect.gen(function* () {
      ...
      expect((yield* SessionFormat.admit({ type: "json_schema", schema: nested(65) }).pipe(Effect.flip)).reason).toBe(
        "schema-too-deep",
      )
    }),
  )
  ...
  it.effect("applies exact byte and depth limits only to the extracted value", () =>
    Effect.gen(function* () {
      ...
      expect((yield* SessionFormat.toolValue({ value: `${exact}x` }).pipe(Effect.flip)).reason).toBe("value-limit")
