...
import { Message, Model } from "@slopcode-ai/llm"
import * as OpenAIChat from "@slopcode-ai/llm/protocols/openai-chat"
import { ModelV2 } from "@slopcode-ai/core/model"
import { ProviderV2 } from "@slopcode-ai/core/provider"
import { SessionMessage } from "@slopcode-ai/core/session/message"
import { AgentAttachment, FileAttachment } from "@slopcode-ai/core/session/prompt"
import { toLLMMessages } from "@slopcode-ai/core/session/runner/to-llm-message"
...
describe("toLLMMessages", () => {
  test("keeps pending custom tool source raw", () => {
    const messages = toLLMMessages(
      [
        new SessionMessage.Assistant({
          id: id("pending-custom"),
          type: "assistant",
          agent: "build",
          ...
          content: [
            new SessionMessage.AssistantTool({
              type: "tool",
              toolType: "custom",
              id: "call-pending-exec",
              name: "exec",
              state: new SessionMessage.ToolStatePending({ status: "pending", input: "42" }),
              time: { created },
            }),
          ],
          time: { created },
    ...
    expect(messages[0]?.content).toMatchObject([
      { type: "tool-call", toolType: "custom", id: "call-pending-exec", name: "exec", input: "42" },
    ])
  })

  test("replays custom tool calls and results with raw source", () => {
    const messages = toLLMMessages(
      [
        new SessionMessage.Assistant({
          id: id("custom"),
