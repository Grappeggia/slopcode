        ...
        withTool(
          ...
          (registry) =>
            Effect.gen(function* () {
              ...
              expect(yield* Effect.promise(() => fs.readFile(path.join(tmp.path, "first.fmt"), "utf8"))).toBe(
                "first\n!",
              )
              expect(yield* Effect.promise(() => fs.readFile(path.join(tmp.path, "second.fmt"), "utf8"))).toBe(
                "second\n!",
              )
            }),
          true,
