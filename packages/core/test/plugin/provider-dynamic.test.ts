...
import { AISDK } from "@slopcode-ai/core/aisdk"
import { EventV2 } from "@slopcode-ai/core/event"
import { ModelV2 } from "@slopcode-ai/core/model"
import { PluginV2 } from "@slopcode-ai/core/plugin"
import { DynamicProviderPlugin } from "@slopcode-ai/core/plugin/provider/dynamic"
...
function tempEntrypoint(source: string) {
  return Effect.acquireRelease(
    Effect.promise(async () => {
      const directory = await fs.mkdtemp(path.join(os.tmpdir(), "slopcode-provider-dynamic-"))
