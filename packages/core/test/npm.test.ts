...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@slopcode/acme")).toBe("@slopcode/acme")
    expect(Npm.sanitize("@slopcode/acme@1.0.0")).toBe("@slopcode/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/slopcode/acme.git"
    const expected = win ? "acme@git+https_//github.com/slopcode/acme.git" : spec
