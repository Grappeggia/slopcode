...
const mutationLayer = (hooks?: FileMutation.HooksInterface) =>
  FileMutation.layer.pipe(
    ...
    Layer.provide(Layer.succeed(FileMutation.Hooks, FileMutation.Hooks.of(hooks ?? { pause: () => Effect.void }))),
  )
...
function postLayer(input: {
  ...
  const events = input.events ? Layer.succeed(EventV2.Service, EventV2.Service.of(input.events)) : EventV2.defaultLayer
    ...
    withTmp((directory) =>
      Effect.gen(function* () {
        ...
        yield* Effect.promise(() =>
          Promise.all([fs.writeFile(approved.canonical, "approved"), fs.writeFile(escaped, "escaped")]),
        )
        ...
        }).pipe(Effect.provide(mutationLayer(hooks)), Effect.forkChild)
  ...
  it.live("formats a private stage and rejects a concurrent user edit", () =>
    withTmp((directory) => {
      ...
      return Effect.gen(function* () {
        ...
        expect(
          yield* Effect.promise(() =>
            fs.stat(path.dirname(stage)).then(
              () => true,
              () => false,
            ),
          ),
        ...
        for (const swap of ["target", "parent"] as const) {
          ...
          yield* Effect.gen(function* () {
            ...
            expect(error).toMatchObject({
              _tag: expect.stringMatching(/^FileMutation\.(?:StaleContent|TargetChanged)Error$/),
            })
          }).pipe(Effect.provide(postLayer({ formatter })))
          expect(yield* Effect.promise(() => fs.readFile(escaped, "utf8"))).toBe("escaped")
          if (swap === "parent") {
    ...
    withTmp((directory) => {
      ...
      return Effect.gen(function* () {
        ...
        yield* events.listen((event) => {
          if (event.type === FileSystem.Event.Edited.type)
            return Effect.sync(() => {
              semantic++
            })
        ...
        const result = yield* (yield* PostMutation.Service).run({
          target: approved,
          intent: "write",
          mutation: Effect.sync(() => {
            primitive++
          }).pipe(Effect.andThen(files.write({ target: approved, content: "changed" }))),
        })
  ...
  it.live("cleans formatter staging and publishes nothing when interrupted", () =>
    withTmp((directory) => {
      ...
      return Effect.gen(function* () {
        ...
        const files = yield* FileMutation.Service
        const fiber = yield* (yield* PostMutation.Service)
          .run({
            target: approved,
            intent: "write",
