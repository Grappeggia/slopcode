    ...
    Effect.acquireRelease(Effect.promise(tmpdir), (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]())).pipe(
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          ...
          expect(yield* store.get(target)).toEqual({})
          expect(
            yield* store.claimLegacy(target, {
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          ...
          const claims = yield* Effect.all(
            [
              first.claimAttempt("winner", "winner-state", "code", 10),
              second.claimAttempt("winner", "winner-state", "code", 10),
            ],
          ...
          expect(
            yield* first.finishExchange(target, "winner", { access_token: "winning", token_type: "Bearer" }, 20),
          ).toEqual({
            won: true,
            cancelled: ["sibling"],
          })
          expect(
            yield* second.finishExchange(target, "winner", { access_token: "late", token_type: "Bearer" }, 21),
          ).toEqual({
            won: false,
            cancelled: [],
          })
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          ...
          for (const data of [
            ...
            expect(yield* MCPOAuthStore.make({ data: tmp.path }).get(target).pipe(Effect.flip)).toMatchObject({
              code: "invalid",
            })
          }
        }),
    ...
    Effect.acquireRelease(Effect.promise(tmpdir), (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]())).pipe(
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          ...
          const output = yield* Effect.promise(() =>
            Promise.all(
              [
                ...
                const process = Bun.spawn(["bun", worker, tmp.path, id!, token!], {
          ...
          expect(
            Object.values((yield* store.get(target)).attempts ?? {})
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          ...
          const invalid = [
            {
              ...
              buckets: {
                [key]: {
                  identity,
                  entry: {
                    ...base.buckets[key]!.entry,
                    tokens: { access_token: "a", token_type: "Bearer", expires_at: "soon" },
                  },
                },
              },
            },
            {
              ...
              buckets: {
                [key]: {
                  identity,
                  entry: { ...base.buckets[key]!.entry, client: { client_id: "c", grant_types: "bad" } },
                },
              },
            },
            {
              ...base,
              buckets: {
                [key]: { identity, entry: { ...base.buckets[key]!.entry, client: { client_id: "c", contacts: [1] } } },
              },
            },
            {
              ...base,
              buckets: {
                [key]: {
                  identity,
                  entry: { ...base.buckets[key]!.entry, client: { client_id: "c", client_uri: "javascript:bad" } },
                },
              },
            },
            {
              ...
              buckets: {
                [key]: {
                  identity,
                  entry: { ...base.buckets[key]!.entry, client: { client_id: "c", client_id_issued_at: -1 } },
                },
              },
            },
            {
              ...
              buckets: {
                [key]: {
                  ...
                  entry: {
                    ...
                    discovery: {
                      authorizationServerUrl: "https://auth.example/",
                      resourceMetadata: { resource: "https://example.com/mcp", scopes_supported: [1] },
                    },
                  },
                },
              },
                ...
                [key]: {
                  ...
                  entry: {
                    ...
                    discovery: {
                      ...
                      resourceMetadata: {
                        resource: "https://example.com/mcp",
                        dpop_bound_access_tokens_required: "no",
                      },
                    },
                  },
                },
          ...
          for (const data of invalid) {
            yield* Effect.promise(() => Bun.write(file, JSON.stringify(data)))
            expect(yield* MCPOAuthStore.make({ data: tmp.path }).get(identity).pipe(Effect.flip)).toMatchObject({
              code: "invalid",
            })
          }
        }),
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          ...
          const lock = yield* Effect.promise(() =>
            Flock.acquire(`mcp-oauth:${file}`, { dir: path.join(tmp.path, ".mcp-oauth-locks") }),
          )
          const waiter = yield* store
            .update(target, (entry) => ({ ...entry, compatibility: "b".repeat(64) }))
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          ...
          const children = Array.from({ length: 12 }, (_, index) =>
            Bun.spawn(["bun", worker, tmp.path, String(index)], {
              cwd: path.dirname(import.meta.dir),
            }),
          )
          expect(yield* Effect.promise(() => Promise.all(children.map((child) => child.exited)))).toEqual(
            Array(12).fill(0),
          )
