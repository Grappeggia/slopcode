...
describe("structured final tool reservation", () => {
  it.effect("rejects persistent and ordinary overlay collisions", () =>
    Effect.gen(function* () {
      ...
      expect((yield* registry.materialize([], {}, { tools: { final_output: ordinary } }).pipe(Effect.flip)).name).toBe(
        "final_output",
      )
    }),
  )
  ...
  it.effect("keeps final_output direct and unavailable to nested CodeMode", () =>
    Effect.gen(function* () {
      const registry = yield* ToolRegistry.Service
      const materialized = yield* registry.materialize([], { mode: "code-only" }, { tools: {} })
