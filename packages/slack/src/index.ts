...
import { createSlopcode, type ToolPart } from "@slopcode-ai/sdk"
...
console.log("🚀 Starting slopcode server...")
const slopcode = await createSlopcode({
  port: 0,
})
console.log("✅ Slopcode server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await slopcode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new slopcode session...")
    const { client, server } = slopcode
    ...
    console.log("✅ Created slopcode session:", createResult.data.id)
  ...
  console.log("📝 Sending to slopcode:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Slopcode response:", JSON.stringify(result, null, 2))
