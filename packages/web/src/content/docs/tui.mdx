...
After an answer, use the same composer to ask follow-ups. Every question reloads the latest main-session context and adds the completed side turns, while the agent, model, and variant selected when you opened the panel stay pinned.

Requests accept up to 32 completed side turns and 64,000 characters per question or answer. The selected model's input limit is also enforced: newer side turns are preferred, older side turns and main history may be omitted, and a request fails before generation if its required content or a read continuation cannot fit.
