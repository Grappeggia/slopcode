...
Con SlopCode puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, SlopCode usa nuestro propio tema `slopcode`.
...
SlopCode viene con varios temas integrados.
...
- Quiere que SlopCode coincida con la apariencia de su terminal
...
SlopCode admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/slopcode/themes/*.json` o `$XDG_CONFIG_HOME/slopcode/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.slopcode/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.slopcode/themes/*.json`
