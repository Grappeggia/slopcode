---
title: Complementos
description: Escriba sus propios complementos para extender SlopCode.
---

Los complementos le permiten extender SlopCode al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de SlopCode.
...
- `.slopcode/plugins/` - Complementos a nivel de proyecto
- `~/.config/slopcode/plugins/` - Complementos globales
...
```json title="slopcode.json"
{
  ...
  "plugin": ["slopcode-helicone-session", "slopcode-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/slopcode/node_modules/`.
...
1. Configuración global (`~/.config/slopcode/slopcode.json`)
2. Configuración del proyecto (`slopcode.json`)
3. Directorio global de complementos (`~/.config/slopcode/plugins/`)
4. Directorio de complementos del proyecto (`.slopcode/plugins/`)
...
```json title=".slopcode/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
SlopCode ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".slopcode/plugins/my-plugin.ts"
...
```js title=".slopcode/plugins/example.js"
...
- `client`: Un cliente SDK slopcode para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar slopcode.
...
```js title=".slopcode/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "slopcode"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio SlopCode, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que slopcode lea archivos `.env`:

```javascript title=".slopcode/plugins/env-protection.js"
...
```javascript title=".slopcode/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a slopcode:

```ts title=".slopcode/plugins/custom-tools.ts"
import { type Plugin, tool } from "@slopcode-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que slopcode puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para slopcode junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://slopcode.dev/docs/sdk) para obtener más detalles.
...
```ts title=".slopcode/plugins/compaction.ts"
import type { Plugin } from "@slopcode-ai/plugin"
...
```ts title=".slopcode/plugins/custom-compaction.ts"
import type { Plugin } from "@slopcode-ai/plugin"
