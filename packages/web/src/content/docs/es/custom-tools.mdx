...
description: Cree herramientas que LLM pueda llamar en slopcode.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de slopcode como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.slopcode/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/slopcode/tools/`.
...
```ts title=".slopcode/tools/database.ts" {1}
...
```ts title=".slopcode/tools/math.ts"
import { tool } from "@slopcode-ai/plugin"
...
```ts title=".slopcode/tools/bash.ts"
import { tool } from "@slopcode-ai/plugin"
...
```ts title=".slopcode/tools/project.ts" {8}
import { tool } from "@slopcode-ai/plugin"
...
```python title=".slopcode/tools/add.py"
import sys
...
```ts title=".slopcode/tools/python-add.ts" {10}
import { tool } from "@slopcode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".slopcode/tools/add.py")
