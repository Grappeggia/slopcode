...
Las herramientas permiten que LLM realice acciones en su código base. SlopCode viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en SlopCode.
...
:::note
Esta herramienta solo está disponible cuando `SLOPCODE_EXPERIMENTAL_LSP_TOOL=true` (o `SLOPCODE_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor SlopCode o cuando la variable de entorno `SLOPCODE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar SlopCode:
