...
Her er alle de innebygde verktøyene som er tilgængelige i SlopCode.
...
:::note
Dette verktøyet er bare tilgængelig når `SLOPCODE_EXPERIMENTAL_LSP_TOOL=true` (eller `SLOPCODE_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger SlopCode-leverandøren eller når miljøvariabelen `SLOPCODE_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
