---
title: Plugins
description: Skriv dine egne plugins for at udvide SlopCode.
---

Plugins giver dig mulighed for at udvide SlopCode ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre SlopCodes standardadfærd.
...
- `.slopcode/plugins/` - Plugins på projektniveau
- `~/.config/slopcode/plugins/` - Globale plugins
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/slopcode/node_modules/`.
...
1. Global konfiguration (`~/.config/slopcode/slopcode.json`)
2. Projektkonfiguration (`slopcode.json`)
3. Global plugin-mappe (`~/.config/slopcode/plugins/`)
4. Projekt plugin bibliotek (`.slopcode/plugins/`)
...
SlopCode kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En slopcode SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide slopcode.
...
:::note
Hvis du bruger SlopCode desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå slopcode i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til slopcode:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som slopcode kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for slopcode sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://slopcode.dev/docs/sdk) for detaljer.
