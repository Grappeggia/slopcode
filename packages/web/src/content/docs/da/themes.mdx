...
Med SlopCode kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger SlopCode vårt eget `slopcode`-tema.
...
SlopCode støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/slopcode/themes/*.json` eller `$XDG_CONFIG_HOME/slopcode/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.slopcode/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.slopcode/themes/*.json`
