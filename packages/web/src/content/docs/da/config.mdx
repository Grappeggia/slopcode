---
title: Konfiguration
description: Ved at bruge SlopCode JSON konfig.
---

Du kan konfigurere SlopCode ved hjælp af en JSON-konfigurationsfil.

---

## Format

SlopCode understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
1. **Fjernkonfiguration** (fra `.well-known/slopcode`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/slopcode/slopcode.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`SLOPCODE_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`slopcode.json` i projekt) - projektspecifikke indstillinger
5. **`.slopcode` mapper** - agent, kommandoer, plugins
6. **Inline config** (`SLOPCODE_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.slopcode` og `~/.config/slopcode` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/slopcode`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale SlopCode-konfiguration i `~/.config/slopcode/slopcode.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `slopcode.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når SlopCode starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `SLOPCODE_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `SLOPCODE_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.slopcode` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.slopcode` mapper, så den **kan tilsidesætte** deres indstillinger.

---

## Skema

Konfigurationsfilen har et skema, der defineres i [**`slopcode.dev/config.json`**](https://slopcode.dev/config.json).
...
Du kan konfigurere serverindstillinger for kommandoerne `slopcode serve` og `slopcode web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din SlopCode-server.
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `slopcode.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din SlopCode-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger SlopCode at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din SlopCode-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/slopcode/agents/` eller `.slopcode/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil SlopCode falde tilbage til `"build"` med en advarsel.

Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`slopcode run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/slopcode/commands/` eller `.slopcode/commands/`. [Learn more here](/docs/commands).
...
SlopCode vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
Som standard slopcode **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
[Plugins](/docs/plugins) udvide SlopCode med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.slopcode/plugins/` eller `~/.config/slopcode/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse SlopCode til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
