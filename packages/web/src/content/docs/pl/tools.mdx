...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. SlopCode posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w SlopCode.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `SLOPCODE_EXPERIMENTAL_LSP_TOOL=true` (lub `SLOPCODE_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy SlopCode lub gdy zmienna środowiskowa `SLOPCODE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu SlopCode:
