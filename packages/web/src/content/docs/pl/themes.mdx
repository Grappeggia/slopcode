...
Dzięki slopcode możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie slopcode używa naszego własnego motywu `slopcode`.
...
slopcode ma kilka wbudowanych motywów.
...
- Chcą, aby SlopCode pasował do wyglądu ich terminala
...
slopcode obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/slopcode/themes/*.json` lub `$XDG_CONFIG_HOME/slopcode/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.slopcode/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.slopcode/themes/*.json`
