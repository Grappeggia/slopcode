...
Umiejętności agenta pozwalają slopcode odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
slopcode przeszukuje te lokalizacje:

- Project config: `.slopcode/skills/<name>/SKILL.md`
- Global config: `~/.config/slopcode/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu slopcode przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.slopcode/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/slopcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.slopcode/skills/git-release/SKILL.md` w ten sposób:
...
slopcode wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `slopcode.json`:
...
**Dla agentów wbudowanych** (w `slopcode.json`):
