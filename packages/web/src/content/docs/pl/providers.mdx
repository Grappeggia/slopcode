---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w slopcode.
---

import config from "../../../../config.mjs"
export const console = config.console

slopcode używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji slopcode.
...
w `~/.local/share/slopcode/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim slopcode
config.
...
SlopCode Zen to lista modeli dostarczonych przez zespół slopcode, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z slopcode. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od SlopCode Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `SlopCode Zen` i przejdź do [slopcode.dev/auth](https://slopcode.dev/zen).
...
Działa jak każdy inny dostawca w slopcode i jest całkowicie opcjonalny w użyciu.

---

## SlopCode Go

SlopCode Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół slopcode, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z slopcode.

1. Uruchom polecenie `/connect` w TUI, wybierz `SlopCode Go` i przejdź do [slopcode.dev/auth](https://slopcode.dev/zen).
...
Aby używać Amazon Bedrock z slopcode:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania slopcode:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `slopcode.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w slopcode nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować slopcode do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod slopcode działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji slopcode.
...
slopcode używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować slopcode
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`slopcode.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Aby korzystać z subskrypcji GitHub Copilot z kodem slopcode:
...
Aby używać Google Vertex AI z slopcode:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania slopcode.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą slopcode, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [slopcode-helicone-session](https://github.com/H2Shami/slopcode-helicone-session), aby automatycznie rejestrować każdą konwersację slopcode jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja slopcode będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować slopcode tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować slopcode tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować slopcode tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem slopcode. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/slopcode).
:::
...
Aby korzystać z Ollama Cloud z slopcode:
...
3. Skopiuj klucz API do użycia w slopcode.
...
6. **Ważne**: Przed użyciem modeli chmurowych w slopcode musisz pobrać informacje o modelu lokalnie:
...
SlopCode Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół slopcode. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>SlopCode Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **SlopCode Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API slopcode.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji slopcode.
...
5. Możesz je także dostosować za pomocą konfiguracji slopcode. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z slopcode:
...
Możesz także dostosować modele za pomocą konfiguracji slopcode. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z slopcode. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `slopcode.json` w katalogu projektu:
...
Pola `limit` pozwalają slopcode zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `slopcode auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację slopcode i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji slopcode.
