---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć slopcode.
---

Wtyczki umożliwiające wyłączenie slopcode poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania slopcode.
...
- `.slopcode/plugins/` - Wtyczki na poziomie projektu
- `~/.config/slopcode/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/slopcode/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/slopcode/slopcode.json`)
2. Konfiguracja projektu (`slopcode.json`)
3. Globalny katalog wtyczek (`~/.config/slopcode/plugins/`)
4. Katalog wtyczek projektu (`.slopcode/plugins/`)
...
slopcode uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z slopcode do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do slopcode.
...
:::note
Jeśli korzystasz z aplikacji komputerowej slopcode, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj slopcode czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do slopcode:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod slopcode. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://slopcode.dev/docs/sdk).
