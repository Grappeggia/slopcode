...
Avec SlopCode, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, SlopCode utilise notre propre thème `slopcode`.
...
SlopCode est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que SlopCode corresponde à l'apparence de leur terminal
...
SlopCode prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/slopcode/themes/*.json` ou `$XDG_CONFIG_HOME/slopcode/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.slopcode/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.slopcode/themes/*.json`
...
{
  "$schema": "https://slopcode.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
