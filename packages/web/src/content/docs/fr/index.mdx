---
title: Introduction
description: Commencez avec SlopCode.
---
...
![SlopCode TUI avec le thème slopcode](../../../assets/lander/screenshot.png)
...
Pour une expérience optimale sur Windows, nous vous recommandons d'utiliser le [Windows Subsystem for Linux (WSL)](/docs/windows-wsl). Il offre de meilleures performances et une compatibilité totale avec les fonctionnalités de SlopCode.
:::
...
Vous pouvez également récupérer le binaire dans les [Releases](https://github.com/teamslop/slopcode/releases).
...
Si vous débutez avec les fournisseurs de LLM, nous vous recommandons d'utiliser [SlopCode Zen](/docs/zen).
C'est une liste curée de modèles qui ont été testés et vérifiés par l'équipe SlopCode.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez slopcode et allez sur [slopcode.dev/auth](https://slopcode.dev/auth).
...
SlopCode dispose d'un _Mode Plan_ qui désactive sa capacité à apporter des modifications et suggère plutôt _comment_ il implémentera la fonctionnalité.
...
:::tip
Donnez à SlopCode beaucoup de contexte et d'exemples pour l'aider à comprendre ce que vous voulez.
:::
...
Les conversations que vous avez avec SlopCode peuvent être [partagées avec votre équipe](/docs/share).
...
Voici un [exemple de conversation](https://slopcode.dev/s/4XP1fce5) avec SlopCode.
...
Pour vous l'approprier, nous vous recommandons de [choisir un thème](/docs/themes), [de personnaliser les raccourcis clavier](/docs/keybinds), de [configurer les formateurs de code](/docs/formatters), de [créer des commandes personnalisées](/docs/commands) ou de jouer avec la [SlopCode Config](/docs/config).
