...
You can add external tools to SlopCode using the _Model Context Protocol_, or MCP. SlopCode supports both local and remote servers.
...
You can define MCP servers in your [SlopCode Config](https://slopcode.dev/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/slopcode` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
SlopCode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, SlopCode will:
...
If the server requires authentication, SlopCode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `slopcode mcp auth <server-name>`.
  ...
  "mcp": {
    "my-oauth-server": {
      ...
      "oauth": {
        ...
        "redirectUri": "http://127.0.0.1:3000/mcp/oauth/callback"
      }
    }
  }
}
...
Set `redirectUri` if your provider requires an exact allowlisted callback URL.
...
The `mcp auth` command will open your browser for authorization. After you authorize, SlopCode will store the tokens securely in `~/.local/share/slopcode/mcp-auth.json`.
...
| `redirectUri`  | String          | Override the OAuth redirect URI used for browser callbacks.                      |
...
Your MCPs are available as tools in SlopCode, alongside built-in tools. So you can manage them through the SlopCode config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect SlopCode to your Sentry account.
