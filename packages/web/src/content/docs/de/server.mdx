...
Der Befehl `slopcode serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein slopcode-Client nutzen kann.
...
| `--mdns-domain` | Custom domain name for mDNS service | `slopcode.local` |
...
Setze `SLOPCODE_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `slopcode`, kann aber mit `SLOPCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `slopcode serve` und `slopcode web`.
...
Wenn du `slopcode` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den slopcode-Server, um slopcode programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `slopcode serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `slopcode serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die SlopCode-[IDE](/docs/ide)-Plugins.
...
Der slopcode-Server stellt folgende APIs bereit.
