...
Mit SlopCode kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet SlopCode das `slopcode`-Theme.
...
SlopCode liefert mehrere eingebaute Themes mit.
...
- SlopCode optisch ans Terminal anpassen wollen
...
SlopCode unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/slopcode/themes/*.json` oder `$XDG_CONFIG_HOME/slopcode/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.slopcode/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.slopcode/themes/*.json`
