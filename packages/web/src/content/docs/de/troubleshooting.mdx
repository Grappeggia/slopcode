...
Wenn SlopCode Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\slopcode\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `slopcode --log-level DEBUG`.

---

## Speicher

slopcode speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/slopcode/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\slopcode`
...
SlopCode Desktop startet im Hintergrund einen lokalen SlopCode-Server (`slopcode-cli`-Sidecar).
...
- Nur macOS: `SlopCode`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/slopcode/slopcode.jsonc` (oder `~/.config/slopcode/slopcode.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/slopcode/slopcode.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\slopcode\slopcode.jsonc`
...
SlopCode kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/slopcode/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\slopcode\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit SlopCode ihn neu aufbauen kann.

1. Beende SlopCode Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/slopcode`
- **Linux**: loesche `~/.cache/slopcode` (oder fuehre aus: `rm -rf ~/.cache/slopcode`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\slopcode`

3. Starte SlopCode Desktop neu.

---

### Server-Verbindungsprobleme beheben

SlopCode Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `slopcode.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `SLOPCODE_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `SLOPCODE_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt SlopCode Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit SlopCode-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

SlopCode Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer SlopCode in den OS-Einstellungen aktiviert sind, und
...
1. Beende SlopCode Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von SlopCode Desktop):

- `slopcode.settings.dat` (desktop default server URL)
- `slopcode.global.dat` und `slopcode.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit SlopCode hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `slopcode models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\slopcode`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. slopcode installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\slopcode`

2. Starte slopcode neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt slopcode, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
slopcode erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
