...
Agent Skills erlauben SlopCode, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
SlopCode sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft SlopCode vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.slopcode/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/slopcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.slopcode/skills/git-release/SKILL.md` zum Beispiel so:
...
SlopCode listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `slopcode.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `slopcode.json`):
