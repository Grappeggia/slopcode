...
CLI, server commands, and interactive TUI sessions work natively in Android Termux on arm64 and x64 when installed from npm with optional dependencies.

Android uses the bundled Rust `ratatui`/`crossterm` TUI instead of OpenTUI/Bun FFI.
