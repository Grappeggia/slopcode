...
CLI and server commands work natively in Android Termux on arm64 and x64 when installed from npm with optional dependencies.

The full OpenTUI renderer is still blocked on Android, so interactive Termux sessions use the bundled Rust sidecar with Linux-like terminal chrome.
...
This is the recommended way to use a richer desktop TUI with a server running elsewhere, including a Termux-hosted backend.
...
Use `--dangerously-skip-permissions` only in trusted automation. It automatically approves permission requests unless they are explicitly denied.
...
Use `--sanitize` to redact sensitive transcript fields, paths, diffs, and file contents before exporting.
...
SlopCode also supports `winget` and direct release binaries, so pick the method that matches how you installed it.
...
| `SLOPCODE_DISABLE_MOUSE`              | boolean | Disable mouse support in the TUI                     |
...
| `SLOPCODE_PURE`                       | boolean | Reduce terminal styling and feature detection        |
...
| `SLOPCODE_DISABLE_EXTERNAL_SKILLS`    | boolean | Disable external `.claude` and `.agents` skills      |
| `SLOPCODE_DISABLE_MODELS_FETCH`       | boolean | Disable fetching models from remote sources          |
| `SLOPCODE_DISABLE_PROJECT_CONFIG`     | boolean | Ignore project config files                          |
...
| `SLOPCODE_ENABLE_QUESTION_TOOL`       | boolean | Enable the question tool for non-interactive clients |
| `SLOPCODE_MODELS_URL`                 | string  | Custom URL for fetching models configuration         |
| `SLOPCODE_MODELS_PATH`                | string  | Load model metadata from a local file                |
...
| `SLOPCODE_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | Enable background subagents and `task_status` |
| `SLOPCODE_EXPERIMENTAL_SCOUT`                   | boolean | Enable reference repo scout tools             |
