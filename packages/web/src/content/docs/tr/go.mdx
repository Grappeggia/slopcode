...
SlopCode Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, SlopCode'daki diğer sağlayıcılar gibi çalışır. SlopCode Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve SlopCode'u kullanmak için bunu kullanmanıza gerek yoktur.
...
SlopCode Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

SlopCode Go, SlopCode'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>SlopCode Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `SlopCode Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye SlopCode Go'ya abone olabilir.
:::
...
SlopCode Go aşağıdaki limitleri içerir:
...
SlopCode yapılandırmanızdaki [model id](/docs/config/#models) formatı `slopcode-go/<model-id>` şeklindedir. Örneğin, Kimi K2.7 Code için yapılandırmanızda `slopcode-go/kimi-k2.7-code` kullanmalısınız.
...
SlopCode Go'yu şu amaçlarla oluşturduk:
...
4. SlopCode ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
