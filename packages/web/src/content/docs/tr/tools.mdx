...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. slopcode yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
slopcode içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `SLOPCODE_EXPERIMENTAL_LSP_TOOL=true` (veya `SLOPCODE_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca slopcode provider kullanılırken veya `SLOPCODE_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

slopcode başlatırken etkinleştirmek için:
