---
title: Eklentiler
description: SlopCode'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek SlopCode'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya SlopCode'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.slopcode/plugins/` - Proje düzeyinde eklentiler
- `~/.config/slopcode/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/slopcode/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/slopcode/slopcode.json`)
2. Project config (`slopcode.json`)
3. Global eklenti dizini (`~/.config/slopcode/plugins/`)
4. Proje eklenti dizini (`.slopcode/plugins/`)
...
SlopCode bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir SlopCode SDK istemcisi.
...
SlopCode'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
SlopCode masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
SlopCode'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca SlopCode'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, SlopCode'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra SlopCode'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://slopcode.dev/docs/sdk) bakın.
