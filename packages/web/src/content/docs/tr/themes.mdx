...
slopcode ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak slopcode kendi `slopcode` temasını kullanır.
...
slopcode birden fazla yerleşik temayla gelir.
...
- slopcode'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
slopcode, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/slopcode/themes/*.json` veya `$XDG_CONFIG_HOME/slopcode/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.slopcode/themes/*.json`
4. **Geçerli çalışma dizini** - `./.slopcode/themes/*.json`
