---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di SlopCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

SlopCode fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire SlopCode avvia la TUI per la directory corrente.
...
Quando usi la TUI di SlopCode, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a SlopCode. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da SlopCode. _Alias_: `/quit`, `/q`
...
Questo è separato da `slopcode.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo SlopCode attende dopo il leader key. Default: `2000`.
...
- `sound_pack` - ID del sound pack da usare. Default: `slopcode.default`.
