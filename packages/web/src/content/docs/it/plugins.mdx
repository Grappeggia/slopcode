---
title: Plugin
description: Scrivi plugin per estendere SlopCode.
---

I plugin ti permettono di estendere SlopCode agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di SlopCode.
...
- `.slopcode/plugins/` - plugin a livello progetto
- `~/.config/slopcode/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/slopcode/node_modules/`.
...
1. Config globale (`~/.config/slopcode/slopcode.json`)
2. Config di progetto (`slopcode.json`)
3. Directory plugin globale (`~/.config/slopcode/plugins/`)
4. Directory plugin di progetto (`.slopcode/plugins/`)
...
SlopCode esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di slopcode per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere slopcode.
...
:::note
Se usi l'app desktop di SlopCode, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a slopcode di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a slopcode:
...
L'helper `tool` crea uno strumento personalizzato che slopcode può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in slopcode insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://slopcode.dev/docs/sdk) per i dettagli.
