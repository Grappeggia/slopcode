...
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Password per l'autenticazione di base (predefinita: `SLOPCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `SLOPCODE_SERVER_USERNAME` o `slopcode`) |
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Attach a un server slopcode in esecuzione (es. http://localhost:4096)                           |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Password per l'autenticazione di base (predefinita: `SLOPCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `SLOPCODE_SERVER_USERNAME` o `slopcode`) |
...
| `SLOPCODE_TUI_CONFIG`                 | string  | Percorso al file di configurazione TUI                      |
...
| Variabile                                       | Tipo    | Descrizione                                             |
| ----------------------------------------------- | ------- | ------------------------------------------------------- |
| `SLOPCODE_EXPERIMENTAL`                         | boolean | Abilita funzionalità sperimentali sotto il flag globale |
...
| `SLOPCODE_EXPERIMENTAL_LSP_TY`                  | boolean | Abilita TY LSP per i file python                        |
| `SLOPCODE_EXPERIMENTAL_PLAN_MODE`               | boolean | Abilita plan mode                                       |
| `SLOPCODE_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | Abilita task subagent in background                     |
| `SLOPCODE_EXPERIMENTAL_EVENT_SYSTEM`            | boolean | Abilita sistema eventi sperimentale                     |
| `SLOPCODE_EXPERIMENTAL_NATIVE_LLM`              | boolean | Abilita percorso nativo di richiesta LLM                |
| `SLOPCODE_EXPERIMENTAL_PARALLEL`                | boolean | Abilita esecuzione parallela della ricerca web          |
| `SLOPCODE_EXPERIMENTAL_SCOUT`                   | boolean | Abilita subagent Scout                                  |
| `SLOPCODE_EXPERIMENTAL_WORKSPACES`              | boolean | Abilita supporto workspace                              |
