---
title: Formatters
description: SlopCode uses language specific formatters.
---

SlopCode can format files after they are written or edited using language-specific formatters. Formatters are disabled by default; enable them in your config before SlopCode will run them.
...
When formatters are enabled, SlopCode will use `prettier` for matching files if your project has `prettier` in `package.json`.

---

## How it works

When SlopCode writes or edits a file and formatters are enabled, it:
...
You can enable and customize formatters through the `formatter` section in your SlopCode config.
