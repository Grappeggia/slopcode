---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u SlopCode.
---

import config from "../../../../config.mjs"
export const console = config.console

SlopCode koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj SlopCode konfiguraciji.
...
u `~/.local/share/slopcode/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem SlopCode
config.
...
SlopCode Zen je lista modela koje je obezbedio SlopCode tim koji su bili
testirano i potvrđeno da dobro radi sa SlopCode. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa SlopCode Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite slopcode i idite na [slopcode.dev/auth](https://slopcode.dev/auth).
...
Radi kao i svaki drugi provajder u SlopCode i potpuno je opcionalan za korištenje.

---

## SlopCode Go

SlopCode Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža SlopCode tim i koji su testirani i verificirani da dobro rade s SlopCode-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `SlopCode Go` i idite na [slopcode.dev/auth](https://slopcode.dev/zen).
...
Da biste koristili Amazon Bedrock s SlopCode:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete slopcode:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `slopcode.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u SlopCode.
:::
...
Možete konfigurirati slopcode za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi slopcode ispravno radio.
   :::
...
Također možete dodati modele kroz svoju slopcode konfiguraciju.
...
da pohrani token u slopcode auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
SlopCode koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate SlopCode
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`slopcode.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `slopcode.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s slopcode:
...
Za korištenje Google Vertex AI s SlopCode:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće slopcode.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem slopcodea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [slopcode-helicone-session](https://github.com/H2Shami/slopcode-helicone-session) da automatski prijavite svaki SlopCode razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki SlopCode razgovor naveden kao zasebna sesija.
...
Možete konfigurirati slopcode za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati slopcode za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati slopcode za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za SlopCode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/slopcode) za detalje.
:::
...
Da biste koristili Ollama Cloud s SlopCode:
...
3. Kopirajte API ključ za korištenje u SlopCode.
...
6. **Važno**: Prije upotrebe modela oblaka u SlopCode, morate lokalno povući informacije o modelu:
...
SlopCode Zen je lista testiranih i verifikovanih modela koju je obezbedio SlopCode tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>SlopCode Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **SlopCode Zen**.

```txt
   /connect
```

3. Unesite svoj SlopCode API ključ.
...
Također možete dodati dodatne modele putem vaše slopcode konfiguracije.
...
5. Također ih možete prilagoditi putem vaše slopcode konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Slopcodeom:
...
Također možete prilagoditi modele kroz svoju slopcode konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s slopcode-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `slopcode.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju SlopCode da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `slopcode auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite SlopCode konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj slopcode konfiguraciji.
