---
title: Konfiguracija
description: Korištenje SlopCode JSON konfiguracije.
---

Možete konfigurirati SlopCode koristeći JSON konfiguracijski fajl.

---

## Format

SlopCode podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/slopcode`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/slopcode/slopcode.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`SLOPCODE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`slopcode.json` u projektu) - postavke specifične za projekat
5. **`.slopcode` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`SLOPCODE_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.slopcode` i `~/.config/slopcode` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/slopcode` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu SlopCode konfiguraciju u `~/.config/slopcode/slopcode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/slopcode/tui.json`.
...
Dodajte `slopcode.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se SlopCode pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `SLOPCODE_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `SLOPCODE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.slopcode` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.slopcode`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`slopcode.dev/config.json`**](https://slopcode.dev/config.json).

TUI konfiguracija koristi [**`slopcode.dev/tui.json`**](https://slopcode.dev/tui.json).
...
Koristite `SLOPCODE_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `slopcode.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `slopcode serve` i `slopcode web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš SlopCode server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `slopcode.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj SlopCode konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, SlopCode pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/slopcode/agents/` ili `.slopcode/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, SlopCode će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`slopcode run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/slopcode/commands/` ili `.slopcode/commands/`. [Saznajte više ovdje](/docs/commands).
...
SlopCode će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, SlopCode **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju SlopCode sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.slopcode/plugins/` ili `~/.config/slopcode/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite SlopCode da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
