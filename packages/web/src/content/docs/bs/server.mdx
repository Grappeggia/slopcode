...
Komanda `slopcode serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji slopcode klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `slopcode.local` |
...
Postavite `SLOPCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `slopcode`, ili postavite `SLOPCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `slopcode serve` i za `slopcode web`.

```bash
SLOPCODE_SERVER_PASSWORD=your-password slopcode serve
```
...
Kada pokrenete `slopcode`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite slopcode server za programsku interakciju sa slopcode-om.
:::

Ova arhitektura omogucava slopcode podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `slopcode serve` da startate standalone server. Ako je slopcode TUI vec pokrenut, `slopcode serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste SlopCode [IDE](/docs/ide) pluginovi.
...
slopcode server izlaže sljedece API-je.
