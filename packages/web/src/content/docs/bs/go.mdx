...
SlopCode Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u SlopCode-u. Pretplatite se na SlopCode Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili SlopCode.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa SlopCode-om.
:::
...
SlopCode Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

SlopCode Go radi kao bilo koji drugi provajder u SlopCode-u.

1. Prijavite se na **<a href={console}>SlopCode Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `SlopCode Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na SlopCode Go.
:::
...
SlopCode Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj SlopCode konfiguraciji
koristi format `slopcode-go/<model-id>`. Na primjer, za Kimi K2.7 Code, koristili biste
`slopcode-go/kimi-k2.7-code` u svojoj konfiguraciji.
...
Napravili smo SlopCode Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz SlopCode koristite i bilo kojeg drugog provajdera.
