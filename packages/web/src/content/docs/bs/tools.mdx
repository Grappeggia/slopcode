...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. SlopCode dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u SlopCode.
...
:::note
Ovaj alat je dostupan samo kada je `SLOPCODE_EXPERIMENTAL_LSP_TOOL=true` (ili `SLOPCODE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz SlopCode provajdera ili kada je varijabla `SLOPCODE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju SlopCode:
