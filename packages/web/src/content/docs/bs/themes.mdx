...
U SlopCode mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, SlopCode koristi nasu `slopcode` temu.
...
SlopCode dolazi sa vise ugradenih tema.
...
- Zele da SlopCode odgovara izgledu njihovog terminala
...
SlopCode podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/slopcode/themes/*.json` ili `$XDG_CONFIG_HOME/slopcode/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.slopcode/themes/*.json`
4. **Trenutni radni direktorij** - `./.slopcode/themes/*.json`
