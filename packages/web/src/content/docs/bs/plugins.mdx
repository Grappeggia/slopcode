---
title: Dodaci
description: Napišite vlastite dodatke za proširenje SlopCode.
---

Dodaci vam omogućavaju da proširite SlopCode spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja SlopCode.
...
- `.slopcode/plugins/` - Dodaci na nivou projekta
- `~/.config/slopcode/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/slopcode/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/slopcode/slopcode.json`)
2. Konfiguracija projekta (`slopcode.json`)
3. Globalni direktorij dodataka (`~/.config/slopcode/plugins/`)
4. Direktorij dodataka projekta (`.slopcode/plugins/`)
...
SlopCode pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Slopcode SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje SlopCode.
...
Spriječite slopcode da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u slopcode:
...
Pomoćnik `tool` kreira prilagođeni alat koji slopcode može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za slopcode zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://slopcode.dev/docs/sdk) za detalje.
