...
SlopCode uses a `leader` key for many keybinds. This avoids conflicts in your terminal.
...
`leader_timeout` controls how long SlopCode waits for the next key after the leader key. It defaults to `2000` milliseconds.
