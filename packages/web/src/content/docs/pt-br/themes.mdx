...
Com o slopcode, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o slopcode usa nosso próprio tema `slopcode`.
...
O slopcode vem com vários temas embutidos.
...
- Querem que o slopcode corresponda à aparência do seu terminal
...
O slopcode suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/slopcode/themes/*.json` ou `$XDG_CONFIG_HOME/slopcode/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.slopcode/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.slopcode/themes/*.json`
