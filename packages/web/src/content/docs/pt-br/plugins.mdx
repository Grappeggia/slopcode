---
title: Plugins
description: Escreva seus próprios plugins para estender o slopcode.
---

Plugins permitem que você estenda o slopcode conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do slopcode.
...
- `.slopcode/plugins/` - Plugins em nível de projeto
- `~/.config/slopcode/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/slopcode/node_modules/`.
...
1. Configuração global (`~/.config/slopcode/slopcode.json`)
2. Configuração do projeto (`slopcode.json`)
3. Diretório de plugins global (`~/.config/slopcode/plugins/`)
4. Diretório de plugins do projeto (`.slopcode/plugins/`)
...
O slopcode executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do slopcode para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o slopcode.
...
:::note
Se você estiver usando o aplicativo desktop slopcode, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o slopcode de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao slopcode:
...
O helper `tool` cria uma ferramenta personalizada que o slopcode pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o slopcode junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://slopcode.dev/docs/sdk) para detalhes.
