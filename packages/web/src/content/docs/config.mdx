...
1. **Remote config** (from `.well-known/slopcode`) - organizational defaults
2. **Global config** (`~/.config/slopcode/slopcode.json`) - user preferences
3. **Custom config** (`SLOPCODE_CONFIG` env var) - custom overrides
4. **Project config** (`slopcode.json` in project) - project-specific settings
5. **`.slopcode` directories** - agents, commands, plugins
6. **Inline config** (`SLOPCODE_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/slopcode/` on macOS) - admin-controlled
...
:::note
The `.slopcode` and `~/.config/slopcode` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Place your global SlopCode config in `~/.config/slopcode/slopcode.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/slopcode/tui.json`.
...
Add `slopcode.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When SlopCode starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
modes, and plugins just like the standard `.slopcode` directory, and should
...
The custom directory is loaded after the global config and `.slopcode` directories, so it **can override** their settings.
...
Drop an `slopcode.json` or `slopcode.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/slopcode/` |
| Linux    | `/etc/slopcode/`                         |
| Windows  | `%ProgramData%\slopcode`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, SlopCode reads managed preferences from the `ai.slopcode.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

SlopCode checks these paths:

1. `/Library/Managed Preferences/<user>/ai.slopcode.managed.plist`
2. `/Library/Managed Preferences/ai.slopcode.managed.plist`

The plist keys map directly to `slopcode.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.slopcode.managed` PayloadType. The SlopCode config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.slopcode.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.slopcode.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.slopcode</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
If not specified, SlopCode will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
Use the `experimental.policies` option to allow or deny SlopCode actions on configured resources. Currently, policies can control which providers SlopCode may use.
...
SlopCode normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, SlopCode omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
SlopCode uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
