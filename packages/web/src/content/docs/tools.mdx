...
Tools allow the LLM to perform actions in your codebase. SlopCode comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
:::note
This tool is only available when `SLOPCODE_EXPERIMENTAL_LSP_TOOL=true` (or `SLOPCODE_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the SlopCode provider or when the `SLOPCODE_ENABLE_EXA` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching SlopCode:

```bash
SLOPCODE_ENABLE_EXA=1 slopcode
```
...
::::note
This tool is only available when `SLOPCODE_EXPERIMENTAL_BACKGROUND_SUBAGENTS=true` (or `SLOPCODE_EXPERIMENTAL=true`).
::::
...
::::note
This tool is only available when `SLOPCODE_EXPERIMENTAL_SCOUT=true` (or `SLOPCODE_EXPERIMENTAL=true`).
::::
...
::::note
This tool is only available when `SLOPCODE_EXPERIMENTAL_PLAN_MODE=true` (or `SLOPCODE_EXPERIMENTAL=true`) in the CLI client.
::::
