...
:::info
Using your Claude Pro/Max subscription in SlopCode is not officially supported by [Anthropic](https://anthropic.com).
:::
...
More documentation on [slopcode-gitlab-auth](https://www.npmjs.com/package/@gitlab/slopcode-gitlab-auth) homepage.
...
{
  "$schema": "https://slopcode.dev/config.json",
  "plugin": ["@gitlab/slopcode-gitlab-plugin"]
}
```
...
1. Head over to [LLM Gateway](https://llmgateway.io/), create an account, and generate an API key.
...
3. Enter your LLM Gateway API key.
...
[Snowflake Cortex](https://docs.snowflake.com/en/user-guide/snowflake-cortex/cortex-rest-api) gives you access to Claude and OpenAI models through an OpenAI-compatible API inside the Snowflake perimeter.

Run the auth command and enter your Snowflake account identifier plus Programmatic Access Token:

```bash
slopcode auth login --provider snowflake-cortex
```

Set your Snowflake account identifier and Programmatic Access Token before starting slopcode:
...
Then select a Snowflake Cortex model:
...
You can also set credentials directly in provider options:
...
Only Claude and OpenAI model families are exposed because slopcode relies on tool calling for its core workflow.
