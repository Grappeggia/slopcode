...
Det er to typer agenter i SlopCode; primære agenter og underagenter.
...
SlopCode kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
SlopCode kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

SlopCode kommer med to innebygde primære agenter og tre innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `slopcode.json`:
...
Hvis ingen temperatur er spesifisert, bruker SlopCode modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale SlopCode-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i SlopCode-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [SlopCode Zen](/docs/zen), vil du bruke `slopcode/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `slopcode models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/teamslop/slopcode).
:::
