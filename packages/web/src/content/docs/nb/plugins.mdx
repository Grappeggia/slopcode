...
description: Skriv dine egne programtillegg for å utvide SlopCode.
---

Plugins lar deg utvide SlopCode ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til SlopCode.
...
- `.slopcode/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/slopcode/node_modules/`.
...
1. Global konfigurasjon (`~/.config/slopcode/slopcode.json`)
2. Prosjektkonfigurasjon (`slopcode.json`)
3. Global plugin-katalog (`~/.config/slopcode/plugins/`)
4. Prosjektpluginkatalog (`.slopcode/plugins/`)
...
SlopCode kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En SlopCode SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide SlopCode.
...
:::note
Hvis du bruker SlopCode-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre SlopCode fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til SlopCode:
...
`tool`-hjelperen lager et tilpasset verktøy som SlopCode kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for SlopCode sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://slopcode.dev/docs/sdk) for detaljer.
