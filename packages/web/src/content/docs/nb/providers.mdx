---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i SlopCode.
---

import config from "../../../../config.mjs"
export const console = config.console

SlopCode bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i SlopCode-konfigurasjonen.
...
i `~/.local/share/slopcode/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i SlopCode
konfig.
...
SlopCode Zen er en liste over modeller levert av SlopCode-teamet som har vært
testet og verifisert for å fungere godt med SlopCode. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med SlopCode Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `SlopCode Zen` og gå til [slopcode.dev/auth](https://slopcode.dev/zen).
...
Det fungerer som alle andre leverandører i SlopCode og er helt valgfritt å bruke.

---

## SlopCode Go

SlopCode Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av SlopCode-teamet som har vært
testet og verifisert for å fungere godt med SlopCode.

1. Kjør kommandoen `/connect` i TUI, velg `SlopCode Go`, og gå til [slopcode.dev/auth](https://slopcode.dev/zen).
...
Slik bruker du Amazon Bedrock med SlopCode:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører SlopCode:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `slopcode.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i slopcode støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere slopcode til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at SlopCode skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom SlopCode-konfigurasjonen.
...
for å lagre token i SlopCode auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
SlopCode bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse SlopCode
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`slopcode.json` fil. Det anbefales også å deaktivere øktdeling.
...
Slik bruker du GitHub Copilot-abonnementet med SlopCode:
...
Slik bruker du Google Vertex AI med SlopCode:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører SlopCode.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom SlopCode, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [slopcode-helicone-session](https://github.com/H2Shami/slopcode-helicone-session) plugin for automatisk å logge hver SlopCode-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver SlopCode-samtale oppført som en separat økt.
...
Du kan konfigurere SlopCode for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere SlopCode for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere SlopCode for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for SlopCode. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/slopcode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med SlopCode:
...
3. Kopier API-nøkkelen for bruk i SlopCode.
...
6. **Viktig**: Før du bruker skymodeller i SlopCode, må du hente modellinformasjonen lokalt:
...
SlopCode Zen er en liste over testede og verifiserte modeller levert av SlopCode-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>SlopCode Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **SlopCode Zen**.

   ```txt
   /connect
   ```

3. Skriv inn SlopCode API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom SlopCode-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom SlopCode-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med SlopCode:
...
Du kan også tilpasse modeller gjennom SlopCode-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med SlopCode. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `slopcode.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar SlopCode forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `slopcode auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk SlopCode-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i SlopCode-konfigurasjonen.
