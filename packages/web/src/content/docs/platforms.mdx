...
Interactive Termux sessions use the bundled Rust TUI built on `ratatui` and `crossterm`.

For Android integrations, install Termux:API from the same source as Termux and install the package tools when you want clipboard or file handoff.

```bash
pkg install termux-api
slopcode doctor android
```
