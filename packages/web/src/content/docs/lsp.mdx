---
title: LSP Servers
description: SlopCode integrates with your LSP servers.
---

SlopCode can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent.
...
:::note
You can disable automatic LSP server downloads by setting the `SLOPCODE_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and slopcode opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your slopcode config.
