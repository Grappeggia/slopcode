---
title: 設定
description: 使用 SlopCode JSON 設定。
---
...
1. **遠端設定**（來自 `.well-known/slopcode`）- 組織預設值
2. **全域設定**（`~/.config/slopcode/slopcode.json`）- 使用者偏好
3. **自訂設定**（`SLOPCODE_CONFIG` 環境變數）- 自訂覆寫
4. **專案設定**（專案中的 `slopcode.json`）- 專案特定設定
5. **`.slopcode` 目錄** - 代理、指令、外掛程式
6. **內嵌設定**（`SLOPCODE_CONFIG_CONTENT` 環境變數）- 執行時覆寫

這意味著專案設定可以覆寫全域預設值，全域設定可以覆寫遠端組織預設值。

:::note
`.slopcode` 和 `~/.config/slopcode` 目錄的子目錄使用**複數名稱**：`agents/`、`commands/`、`modes/`、`plugins/`、`skills/`、`tools/` 和 `themes/`。為了向後相容，也支援單數名稱（例如 `agent/`）。
:::
...
自訂目錄在全域設定和 `.slopcode` 目錄之後載入，因此**可以覆寫**它們的設定。

---

## Schema

設定檔具有在 [**`slopcode.dev/config.json`**](https://slopcode.dev/config.json) 中定義的 Schema。
...
使用 `SLOPCODE_TUI_CONFIG` 指向自訂 TUI 設定檔。

`slopcode.json` 中的舊版 `theme`、`keybinds` 和 `tui` 鍵已被棄用，並將在可能的情況下自動遷移。
...
- `mdns` - 啟用 mDNS 服務探索。這允許網路上的其他裝置發現您的 SlopCode 伺服器。
- `mdnsDomain` - mDNS 服務的自訂網域名稱。預設為 `slopcode.local`。適用於在同一網路上執行多個實例的情境。
...
`small_model` 選項為標題生成等輕量級任務設定單獨的模型。預設情況下，如果您的供應商有更便宜的模型可用，SlopCode 會嘗試使用該模型，否則會回退到您的主模型。
...
您還可以使用 `~/.config/slopcode/agents/` 或 `.slopcode/agents/` 中的 Markdown 檔案定義代理。[在此了解更多](/docs/agents)。
...
預設代理必須是主代理（不能是子代理）。可以是內建代理（如 `"build"` 或 `"plan"`），也可以是您定義的[自訂代理](/docs/agents)。如果指定的代理不存在或是子代理，SlopCode 將回退到 `"build"` 並發出警告。
...
您還可以使用 `~/.config/slopcode/commands/` 或 `.slopcode/commands/` 中的 Markdown 檔案定義指令。[在此了解更多](/docs/commands)。
...
[外掛程式](/docs/plugins)透過自訂工具、掛鉤和整合來擴展 SlopCode。
...
當您希望限制 SlopCode 僅使用特定供應商，而不是逐一停用其他供應商時，此選項非常有用。
