...
SlopCode Go is a low cost subscription — **$5 for your first month**, then **$10/month** — that gives you reliable access to popular open coding models.

Go works like any other provider in SlopCode. You subscribe to SlopCode Go and
...
SlopCode Go gives you access to these models for **$5 for your first month**, then **$10/month**.

---

## How it works

SlopCode Go works like any other provider in SlopCode.

1. You sign in to **<a href={console}>SlopCode Zen</a>**, subscribe to Go, and
   copy your API key.
2. You run the `/connect` command in the TUI, select `SlopCode Go`, and paste
   your API key.
...
SlopCode Go includes the following limits:
...
uses the format `slopcode-go/<model-id>`. For example, for Kimi K2.7 Code, you would
use `slopcode-go/kimi-k2.7-code` in your config.
...
4. Have **no lock-in** by allowing you to use any other provider with SlopCode as well.
