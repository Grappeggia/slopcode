import {
  ag as ROUTE_TYPE_HEADER,
  W as REROUTE_DIRECTIVE_HEADER,
  D as DEFAULT_404_COMPONENT,
  A as AstroError,
  au as ActionNotFoundError,
  P as bold,
  av as red,
  aw as yellow,
  ax as dim,
  ay as blue,
  al as clientAddressSymbol,
  az as LocalsNotAnObject,
  aA as REROUTABLE_STATUS_CODES,
  ap as responseSentSymbol,
  aB as getAugmentedNamespace,
  aC as commonjsGlobal,
} from "./astro/server_CGASYgNY.mjs"
import {
  D as DEFAULT_404_ROUTE,
  e as default404Instance,
  f as ensure404Route,
} from "./astro-designed-error-pages_CW2lTvND.mjs"
...
} from "./path_Cvt6sEOY.mjs"
import {
  r as requestIs404Or500,
  i as isRequestServerIsland,
  n as notFound,
  a as normalizeTheLocale,
  b as redirectToFallback,
  c as redirectToDefaultLocale,
  e as requestHasLocale,
...
} from "./index_BSV4J_2a.mjs"
import { N as NOOP_MIDDLEWARE_FN } from "./noop-middleware_DR3v5wNA.mjs"
...
function tokensToRegexp(tokens, keys, options) {
  ...
  if (end) {
    ...
    var endToken = tokens[tokens.length - 1]
    var isEndDelimited =
      typeof endToken === "string" ? delimiterRe.indexOf(endToken[endToken.length - 1]) > -1 : endToken === undefined
    if (!strict) {
...
function requireSuperstatic() {
  ...
  function replaceSegments(segments, hasItemSegments, destination, isRedirect, internalParamNames) {
    ...
    const canNeedReplacing =
      (destination.includes(":") && namedSegments.length > 0) || hasItemSegments.length > 0 || !isRedirect
    if (!canNeedReplacing) {
      return destination
    }
    ...
    const needsQueryUpdating =
      ...
      !isRedirect &&
      !paramKeys.some((param) => !(internalParamNames && internalParamNames.includes(param)) && destParams.has(param))
...
function requireMerge() {
  ...
  function mergeRoutes({ userRoutes, builds }) {
    ...
    const outputRoutes = []
    const uniqueHandleValues = /* @__PURE__ */ new Set([null, ...userHandleMap.keys(), ...builderHandleMap.keys()])
...
function requireDist$1() {
  ...
  ;(function (module) {
    ...
    function normalizeRoutes(inputRoutes) {
      ...
      const error =
        errors.length > 0 ? createError("invalid_route", errors, "https://vercel.link/routes-json", "Learn More") : null
      return { routes, error }
    }
