...
} from "./Code_CNdM9E30.mjs"
import { aG as requireCssesc, O as getDefaultExportFromCjs } from "./astro/server_CGASYgNY.mjs"
...
function requirePostcssNested() {
  ...
  postcssNested$1.exports = (opts = {}) => {
    let bubble = atruleNames(["media", "supports", "layer", "container", "starting-style"], opts.bubble)
...
function parseOptions(
  ...
  delimitedValues.forEach(({ index, fullMatch: raw, key, value, valueStartDelimiter, valueEndDelimiter }) => {
    inputWithoutDelimited =
      inputWithoutDelimited.slice(0, index) + " ".repeat(raw.length) + inputWithoutDelimited.slice(index + raw.length)
...
function renderLineToAst({ line, lineIndex, gutterElements, ...restContext }) {
  ...
  const renderedGutterElements = sortedGutterElements.map(({ pluginName, gutterElement }) => {
    try {
      ...
      const msg = error instanceof Error ? error.message : error
      throw new Error(`Plugin "${pluginName}" failed to render a gutter element. Error message: ${msg}`, {
        cause: error,
      })
    }
  })
  ...
  return () => {
    ...
    const renderedGutterElements = sortedGutterElements.map(({ pluginName, gutterElement }) => {
      try {
        ...
        const msg = error instanceof Error ? error.message : error
        throw new Error(`Plugin "${pluginName}" failed to render a gutter element placeholder. Error message: ${msg}`, {
          cause: error,
        })
      }
    })
...
async function renderBlock({ codeBlock, groupContents, locale, config, plugins, cssVar, cssVarName, styleVariants }) {
  const state = {
...
var ExpressiveCodeLine = class {
  ...
  editText(columnStart, columnEnd, newText) {
    if (columnStart !== void 0 && !isNumber(columnStart)) throw newTypeError("number", columnStart)
    if (columnEnd !== void 0 && !isNumber(columnEnd)) throw newTypeError("number", columnEnd)
...
async function renderGroup({ input, options, defaultLocale, config, plugins, cssVar, cssVarName, styleVariants }) {
...
function binarySearch({
  ...
  while (((mid = (low + high) / 2), iterations < maxIterations)) {
    ...
    const resultIsInPreferredDirection =
      preferHigher === void 0 ? true : preferHigher ? currentValue > targetValue : currentValue < targetValue
    ...
    return withParsedColor(
      ...
      (backgroundColor) => {
        ...
        if (curContrast < minContrast) {
          ...
          if (contrastWithoutAlpha < minContrast) {
            newColor = toTinyColor(
              changeLuminanceToReachColorContrast(toHexColor(newColor), hexBackgroundColor, minContrast),
            )
        ...
        if (curContrast < minContrast || curContrast > maxContrast) {
          newColor = toTinyColor(
            changeAlphaToReachColorContrast(toHexColor(newColor), hexBackgroundColor, minContrast, maxContrast),
          )
        }
  ...
  function resolveColor(unresolvedColor) {
    ...
    if (typeof unresolvedColor === "string") {
      if (unresolvedColor.startsWith("#")) {
        ...
        if (referencedColor !== void 0) resolved = resolveColor(referencedColor)
      }
  ...
  defaultValues: {
    ...
    gutterForeground: ({ theme, resolveSetting }) =>
      ensureColorContrastOnBackground(
        theme.colors["editorLineNumber.foreground"] || resolveSetting("codeForeground"),
...
function getCoreBaseStyles({ cssVar, useStyleReset, useThemedScrollbars, useThemedSelectionColors }) {
...
function resolveStyleSettings({ theme, styleVariantIndex, plugins, styleOverrides }) {
...
var ExpressiveCodeEngine = class {
  ...
  constructor(config) {
    ...
    this.themes = Array.isArray(config.themes)
      ...
      : config.themes
        ? [config.themes]
        : [new ExpressiveCodeTheme(github_dark_default), new ExpressiveCodeTheme(github_light_default)]
    this.minSyntaxHighlightingColorContrast = config.minSyntaxHighlightingColorContrast ?? 5.5
    this.useDarkModeMediaQuery =
      config.useDarkModeMediaQuery ?? (this.themes.length === 2 && this.themes[0].type !== this.themes[1].type)
  ...
  async getBaseStyles() {
    ...
    for (const plugin of this.plugins) {
      ...
      const resolvedStyles =
        typeof plugin.baseStyles === "function" ? await plugin.baseStyles(resolverContext) : plugin.baseStyles
  ...
  async getThemeStyles() {
    ...
    const renderDeclarations = (declarations) =>
      [...declarations].map(([varName, varValue]) => `${varName}:${varValue}`).join(";")
    const { cssVarDeclarations: baseVars, theme: baseTheme } = this.styleVariants[0]
    const baseThemeSelector =
      this.themeCssSelector && this.themeCssSelector(baseTheme, { styleVariants: this.styleVariants })
    const notBaseThemeSelector = baseThemeSelector ? `:not(${baseThemeSelector})` : ""
    const baseThemeBlockInsideAlternateThemeRoot =
      notBaseThemeSelector && `${this.themeCssRoot}${notBaseThemeSelector} &${baseThemeSelector}`
    ...
    if (this.themeCssSelector !== false) {
      for (const { theme, cssVars, coreStyles } of alternateVariants) {
        const themeSelector =
          this.themeCssSelector && this.themeCssSelector(theme, { styleVariants: this.styleVariants })
  ...
  async getJsModules() {
    ...
    for (const plugin of this.plugins) {
      const pluginModules =
        typeof plugin.jsModules === "function" ? await plugin.jsModules(this.getResolverContext()) : plugin.jsModules
  ...
  defaultValues: {
    frames: {
      shadowColor: ({ theme, resolveSetting }) =>
        theme.colors["widget.shadow"] || multiplyAlpha(resolveSetting("borderColor"), 0.75),
      ...
      terminalTitlebarBackground: ({ theme }) =>
        theme.colors["titleBar.activeBackground"] || theme.colors["editorGroupHeader.tabsBackground"],
...
function pluginFrames(options = {}) {
  ...
  return {
    ...
    jsModules: options.showCopyToClipboardButton
      ? [copy_js_module_min_default.replace(/\[SELECTOR\]/g, ".expressive-code .copy button")]
      : void 0,
    hooks: {
      ...
      postprocessRenderedBlock: ({ codeBlock, renderData, locale }) => {
        ...
        const visibleTitle =
          (frame !== "none" && titleText) || isTerminal ? [h("span", { className: "title" }, titleText || "")] : []
        const screenReaderTitle =
          !titleText && isTerminal ? [h("span", { className: "sr-only" }, texts.terminalWindowFallbackTitle)] : []
        const extraElements = []
    ...
    hooks: {
      ...
      preprocessMetadata: ({ codeBlock, cssVar }) => {
        ...
        codeBlock.props.useDiffSyntax =
          codeBlock.metaOptions.getBoolean("useDiffSyntax") ?? codeBlock.props.useDiffSyntax
        MarkerTypeOrder.forEach((markerType) => {
          toDefinitionsArray(codeBlock.props[markerType]).forEach((definition) => {
      ...
      preprocessCode: ({ codeBlock, cssVar }) => {
        if (codeBlock.language === "diff" || codeBlock.props.useDiffSyntax) {
          ...
          const couldBeRealDiffFile = lines
            .slice(0, 4)
            .some((line) => line.text.match(/^([*+-]{3}\s|@@\s|[0-9,]+[acd][0-9,]+\s*$)/))
        ...
        codeBlock.getLines().forEach((line) => {
          ...
          for (const annotation of annotations) {
            ...
            if (fullLineMarker) {
              if (MarkerTypeOrder.indexOf(annotation.markerType) < MarkerTypeOrder.indexOf(fullLineMarker.markerType))
                continue
              if (
                AnnotationRenderPhaseOrder.indexOf(annotation.renderPhase || "normal") <
                AnnotationRenderPhaseOrder.indexOf(fullLineMarker.renderPhase || "normal")
              )
                continue
            }
          ...
          styleVariants.forEach((styleVariant, styleVariantIndex) => {
            const fullLineMarkerBgColor =
              (fullLineMarker &&
                styleVariant.resolvedStyleSettings.get(markerBgColorPaths[fullLineMarker.markerType])) ||
              "transparent"
            ...
            textColors.forEach((textColor) => {
              ...
              if (textFgColor === void 0 || textStart === void 0 || textEnd === void 0) return
              markers.forEach((marker) => {
                ...
                const inlineMarkerBgColor =
                  (marker.inlineRange &&
                    styleVariant.resolvedStyleSettings.get(markerBgColorPaths[marker.markerType])) ||
                  "transparent"
