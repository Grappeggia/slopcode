...
There are two types of agents in SlopCode; primary agents and subagents.
...
SlopCode comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
SlopCode comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

SlopCode comes with two built-in primary agents and two built-in subagents.
...
Subagents can run in the background when `SLOPCODE_EXPERIMENTAL_BACKGROUND_SUBAGENTS=true` (or `SLOPCODE_EXPERIMENTAL=true`). This is best for independent research, long codebase scans, or work that can continue while the main session handles another task.

A background `task` call returns a `task_id` immediately. Use `task_status` with that ID to poll later, or set `wait: true` when you want to wait for completion.

Background subagents still use the same agent permissions as foreground subagents. They do not bypass `task`, `bash`, `edit`, or external-directory approval rules.
...
Configure agents in your `slopcode.json` config file:
...
- Global: `~/.config/slopcode/agents/`
- Per-project: `.slopcode/agents/`
...
If no temperature is specified, SlopCode uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global SlopCode config and the project specific config.
...
The model ID in your SlopCode config uses the format `provider/model-id`. For example, if you're using [SlopCode Zen](/docs/zen), you would use `slopcode/gpt-5.1-codex` for GPT 5.1 Codex.
...
```json title="slopcode.json" {3-6,9-12}
{
  "$schema": "https://slopcode.dev/config.json",
  "tools": {
    "write": true,
...
```json title="slopcode.json" {3-5,8-10}
{
  "$schema": "https://slopcode.dev/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Run `slopcode models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](http://github.com/teamslop/slopcode).
:::
...
```markdown title="~/.config/slopcode/agents/docs-writer.md"
---
...
```markdown title="~/.config/slopcode/agents/security-auditor.md"
---
