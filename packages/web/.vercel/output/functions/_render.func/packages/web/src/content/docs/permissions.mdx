...
SlopCode uses the `permission` config to decide whether a given action should run automatically, prompt you, or be blocked.
...
Use `external_directory` to allow tool calls that touch paths outside the working directory where SlopCode was started. This applies to any tool that takes a path as input (for example `read`, `edit`, `list`, `glob`, `grep`, and many `bash` commands).
...
SlopCode permissions are keyed by tool name, plus a couple of safety guards:
...
- `followup_recommendations` — suggested next actions (non-mutating)
- `repo_clone`, `repo_overview` — scout/reference repository tools
- `plan_permissions` — permission forecasts before leaving plan mode
- `task_status` — background subagent status checks
...
If you don’t specify anything, SlopCode starts from permissive defaults:
...
When SlopCode prompts for approval, the UI offers three outcomes:

- `once` — approve just this request
- `always` — approve future requests matching the suggested patterns (for the rest of the current SlopCode session)
...
In the TUI, multiple pending requests are grouped into a review flow so you can clear a batch instead of clicking through one dialog at a time.

Build-heavy runs can also surface preapproval choices for repeated safe commands before the next step starts.

The TUI can also auto-accept edit prompts for the current session. This shortcut only replies to `edit` permission requests, and the configured rules still decide whether those edits are allowed, denied, or must be asked.

---

## Forecast access

Plan mode can queue likely build-mode permissions before switching to implementation. The `plan_permissions` forecast lets you review expected `bash`, `edit`, or external-directory needs early.

Forecasting does not grant the underlying tool permissions by itself. It only prepares the review flow so you can approve, narrow, or reject likely requests before build mode starts.

---

## Review

Use `queue_mode: "serial"` when you want approvals to stay in step with queued prompts.

This keeps pauses, resumes, and batched reviews predictable during longer sessions.
...
```markdown title="~/.config/slopcode/agents/review.md"
---
