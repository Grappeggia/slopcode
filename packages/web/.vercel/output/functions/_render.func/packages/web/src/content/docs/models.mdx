...
Most popular providers are preloaded by default. If you've added the credentials for a provider through the `/connect` command, they'll be available when you start SlopCode.
...
The model dialog keeps recent and favorite models near the top. Use the favorite shortcut to pin a model, the provider shortcut to jump across connected providers, and the show-all shortcut when you need models hidden by the latest-only filter.

---

## Complete prompts

Prompt autocomplete can use a different model from the one answering your chat.

```bash frame="none"
/models-completion
```

Pick a small, fast, text-capable model for inline suggestions while keeping the active session on the model you want for reasoning and tool calls. Choose `Automatic` to let SlopCode route autocomplete to its built-in fast model for each provider, or `Use selected model` to reuse the current chat model.
...
Here are several models that work well with SlopCode, in no particular order. (This is not an exhaustive list nor is it necessarily up to date):
...
Here the full ID is `provider_id/model_id`. For example, if you're using [SlopCode Zen](/docs/zen), you would use `slopcode/gpt-5.1-codex` for GPT 5.1 Codex.
...
```jsonc title="slopcode.jsonc" {7-12,19-24}
{
  "$schema": "https://slopcode.dev/config.json",
  "provider": {
    "openai": {
...
```jsonc title="slopcode.jsonc" {6-21}
{
  "$schema": "https://slopcode.dev/config.json",
  "provider": {
    "slopcode": {
...
Many models support multiple variants with different configurations. SlopCode ships with built-in default variants for popular providers.

### Built-in variants

SlopCode ships with default variants for many providers:
...
```jsonc title="slopcode.jsonc" {7-18}
{
  "$schema": "https://slopcode.dev/config.json",
  "provider": {
    "openai": {
...
When SlopCode starts up, it checks for models in the following priority order:

1. The `--model` or `-m` command line flag. The format is the same as in the config file: `provider_id/model_id`.

2. The model list in the SlopCode config.
