...
import { getSlopcodeModeStack, SLOPCODE_BASE_MODE, SlopcodeKeymapProvider, registerSlopcodeKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerSlopcodeKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when slopcode mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getSlopcodeModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getSlopcodeModeStack(keymap).push("autocomplete")
