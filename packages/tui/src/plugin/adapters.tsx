import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@slopcode-ai/plugin/tui"
...
import type { useSlopcodeKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useSlopcodeKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getSlopcodeModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getSlopcodeModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
