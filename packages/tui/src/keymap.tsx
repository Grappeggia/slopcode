...
export const SLOPCODE_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const SLOPCODE_MODE_KEY = "slopcode.mode"

export const SlopcodeKeymapProvider = KeymapProvider
export const useSlopcodeKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type SlopcodeModeStack = ReturnType<typeof createSlopcodeModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, SlopcodeModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createSlopcodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(SLOPCODE_MODE_KEY, SLOPCODE_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(SLOPCODE_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(SLOPCODE_MODE_KEY, stack.at(-1)?.mode ?? SLOPCODE_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? SLOPCODE_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(SLOPCODE_MODE_KEY, undefined)
...
export function useSlopcodeModeStack() {
  return getSlopcodeModeStack(useSlopcodeKeymap())
}

export function getSlopcodeModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Slopcode mode stack is not registered for this keymap")
  return value
}
...
export function registerSlopcodeKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createSlopcodeModeStack(keymap)
...
export function useCommandSlashes(): Accessor<readonly CommandSlashEntry[]> {
  const keymap = useSlopcodeKeymap()
