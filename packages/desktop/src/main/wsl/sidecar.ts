...
import { type WslCommandLine, resolveWslSlopcode, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const slopcode = await resolveWslSlopcode(distro)
  if (!slopcode) throw new Error(`SlopCode is not installed in ${distro}`)
  ...
  const script = [
    ...
    "export PATH",
    "export WSLENV=",
    "export SLOPCODE_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export SLOPCODE_CLIENT=desktop",
    `export SLOPCODE_SERVER_USERNAME=${shellEscape(username)}`,
    `export SLOPCODE_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(slopcode)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
