...
let releaseSlopcodeResolve: (() => void) | undefined
...
test("rejects an update that did not install the desktop version", () => {
  expect(() => expectSlopcodeVersion("1.16.2", "1.16.2")).not.toThrow()
  expect(() => expectSlopcodeVersion("1.14.35", "1.16.2")).toThrow(
    "SlopCode update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating SlopCode", () => {
  expect(
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.slopcode/bin/slopcode",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, slopcodeChecks: {} })
})
...
test("ignores stale background SlopCode checks after removing a WSL server", async () => {
  persistedServers = []
  releaseSlopcodeResolve = undefined
  ...
  await waitFor(() => !!releaseSlopcodeResolve)
  await controller.removeServer("wsl:Debian")
  releaseSlopcodeResolve?.()
  await new Promise((resolve) => setTimeout(resolve, 0))

  expect(controller.getState().servers).toEqual([])
  expect(controller.getState().slopcodeChecks).toEqual({})
})

test("ignores stale startup SlopCode checks after removing a WSL server", async () => {
...
function testControllerOptions() {
  return {
    ...
    resolveSlopcode: async () => {
      await new Promise<void>((resolve) => {
        releaseSlopcodeResolve = resolve
      })
      return "/home/me/.slopcode/bin/slopcode"
    },
  }
}
