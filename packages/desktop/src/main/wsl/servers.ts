...
import { expectSlopcodeVersion, pendingRestartAfterWslInstall, wslServerIdsToStartOnInitialize } from "./startup"
...
type WslServersControllerOptions = {
  ...
  resolveSlopcode?: typeof resolveWslSlopcode
...
export function createWslServersController(
  ...
  const setSlopcodeCheck = (distro: string, check: WslSlopcodeCheck) => {
    setState({
      slopcodeChecks: {
        ...state.slopcodeChecks,
        [distro]: check,
      },
    })
  }

  const checkSlopcode = async (distro: string, opts?: { signal?: AbortSignal }) => {
    const resolved = await (options?.resolveSlopcode ?? resolveWslSlopcode)(distro, opts)
    const version = resolved
      ? await (options?.readCommandVersion ?? readWslCommandVersion)(resolved, distro, opts)
      : null
    return slopcodeCheck(distro, resolved, version, appVersion)
  }

  const refreshSlopcodeCheck = async (distro: string, opts?: { signal?: AbortSignal }) => {
    setSlopcodeCheck(distro, await checkSlopcode(distro, opts))
  }
  ...
  const refreshSlopcodeCheckBackground = (id: string, distro: string) => {
    void checkSlopcode(distro)
      .then((check) => {
        if (!hasServer(id, distro)) return
        setSlopcodeCheck(distro, check)
      })
      .catch((error) => {
        const message = error instanceof Error ? error.message : String(error)
        logger?.error("wsl slopcode check failed", { id, distro, message })
      })
  }

  const refreshSlopcodeChecks = async () => {
    await Promise.all(
      state.servers.map((item) =>
        checkSlopcode(item.config.distro)
          .then((check) => {
            if (!hasServer(item.config.id, item.config.distro)) return
            setSlopcodeCheck(item.config.distro, check)
          })
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl slopcode check failed", {
              id: item.config.id,
  ...
  const startServer = async (id: string) => {
    ...
    try {
      ...
      refreshSlopcodeCheckBackground(id, item.config.distro)
  ...
  return {
    ...
    async probeSlopcode(name: string) {
      await runJob({ kind: "probe-slopcode", distro: name, startedAt: Date.now() }, async (abort) => {
        await refreshSlopcodeCheck(name, { signal: abort.signal })
      })
    },

    async installSlopcode(name: string) {
      await runJob({ kind: "install-slopcode", distro: name, startedAt: Date.now() }, async (abort) => {
        const result = await installWslSlopcode(appVersion, name, { signal: abort.signal })
        if (result.code !== 0) {
          throw new Error(summarize(result.stderr || result.stdout) || "SlopCode installation failed")
        }
        await refreshSlopcodeCheck(name, { signal: abort.signal })
        expectSlopcodeVersion(state.slopcodeChecks[name]?.version ?? null, appVersion, name)
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.slopcodeChecks, distro) : {}),
      })
    },
...
function slopcodeCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "slopcode is not installed in this distro",
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "slopcode is installed but could not run",
    }
  }
  return {
    distro,
