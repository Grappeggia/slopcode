import { SessionMessage } from "@slopcode-ai/schema/session-message"
import { SessionInput } from "@slopcode-ai/schema/session-input"
import { PromptInput } from "@slopcode-ai/schema/prompt-input"
import { Session } from "@slopcode-ai/schema/session"
import { Project } from "@slopcode-ai/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@slopcode-ai/schema/schema"
import { Workspace } from "@slopcode-ai/schema/workspace"
...
import { Agent } from "@slopcode-ai/schema/agent"
import { Model } from "@slopcode-ai/schema/model"
import { Location } from "@slopcode-ai/schema/location"
import { Revert } from "@slopcode-ai/schema/revert"
import { SessionEvent } from "@slopcode-ai/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this SlopCode process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this SlopCode process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
