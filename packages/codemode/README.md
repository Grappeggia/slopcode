...
{
  "dependencies": {
    "@slopcode-ai/codemode": "workspace:*"
  }
}
```
...
```ts
import { CodeMode, Tool } from "@slopcode-ai/codemode"
...
```ts
import { CodeMode, OpenAPI } from "@slopcode-ai/codemode"
...
const runtime = CodeMode.make({ tools: { slopcode: api.tools } })
...
The default budget is 2,000 estimated tokens (characters / 4, the same heuristic Slopcode uses). Override it when constructing a runtime:
...
No limit has a default, on purpose: execution budgets are host policy, not library policy - a host that wants a bound sets one; a host that can interrupt the execution fiber (as Slopcode does on user cancel) may set no timeout, and a host with its own tool-output truncation (as Slopcode has) may leave `maxOutputBytes` unset. A host with neither should set `maxOutputBytes`, or oversized results silently flood model context.
...
CodeMode does not bound source size, interpreter operation count, collection size, or process memory. Parsing and TypeScript transpilation happen before the timeout-wrapped interpretation, and `maxOutputBytes` bounds model-facing output rather than intermediate allocations. The cooperative timeout is not process isolation; hosts that accept untrusted or potentially large source must apply any required input, memory, and process isolation policies outside this package.
...
```ts
import { toolError } from "@slopcode-ai/codemode"
