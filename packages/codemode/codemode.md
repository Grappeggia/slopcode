...
- **`packages/codemode` (`@slopcode-ai/codemode`)** - the generic, host-agnostic runtime:
...
- Generic CodeMode lives in its own package: `@slopcode-ai/codemode` (this repository's scope convention).
...
- `package.json`: name `@slopcode-ai/codemode`, deps `acorn@8.16.0`, `typescript: catalog:`,
...
- Tests converted vitest -> `bun:test`. Only src change from verbatim: the `CurrentToolCall`
  Context.Service key string renamed to `@slopcode-ai/codemode/CurrentToolCall`.
...
- **Deletions/deps**: `src/session/rune/` (all five files) and
  ...
  `"@slopcode-ai/codemode": "workspace:*"` added; `bun install` run (lockfile updated).
