...
export namespace SessionCompaction {
  ...
  export async function process(input: {
    ...
    if (input.overflow) {
      ...
      const hasContent =
        replay && messages.some((m) => m.info.role === "user" && !m.parts.some((part) => part.type === "compaction"))
      if (!hasContent) {
        replay = undefined
...
---`
    ...
    if (result === "continue" && input.auto) {
      const created = Math.max(Date.now(), (processor.message.time.completed ?? processor.message.time.created) + 1)
      if (replay) {
