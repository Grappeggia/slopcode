...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const prompt = Effect.fn("SessionControl.prompt")(function* (input: SessionPrompt.PromptInput) {
      ...
      yield* runtime.assert({ sessionID: input.sessionID, owner: "v2", epoch: info.epoch })
    ...
    const cancel = Effect.fn("SessionControl.cancel")(function* (sessionID: SessionID) {
      ...
      yield* runtime.assert({ sessionID, owner: "v2", epoch: info.epoch })
...
export const defaultLayer = layer.pipe(
  Layer.provide(Layer.mergeAll(SessionRuntime.defaultLayer, SessionV2.defaultLayer, SessionPrompt.defaultLayer)),
)

const sessions = LayerNode.make(SessionV2.defaultLayer, [])

export const node = LayerNode.make(layer, [SessionRuntime.node, sessions, SessionPrompt.node])
