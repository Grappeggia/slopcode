...
const USER_AGENT = `slopcode/${InstallationVersion}`
...
export const prepare = Effect.fn("LLMRequestPrep.prepare")(function* (input: PrepareInput) {
  ...
  const slopcodeProjectID = input.model.providerID.startsWith("slopcode")
  ...
  return {
    ...
    headers: {
      ...(input.model.providerID.startsWith("slopcode")
        ? {
            ...(slopcodeProjectID ? { "x-slopcode-project": slopcodeProjectID } : {}),
            "x-slopcode-session": input.sessionID,
            "x-slopcode-request": input.user.id,
            "x-slopcode-client": input.flags.client,
