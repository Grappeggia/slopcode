...
import type { LLMClientShape } from "@slopcode-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("slopcode"))
    return { type: "unsupported", reason: "provider is not openai, slopcode, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @slopcode-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains slopcode-owned. The native runtime only adapts
      // the @slopcode-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
