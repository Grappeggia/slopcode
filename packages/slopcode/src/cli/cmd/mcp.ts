...
export const McpListCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.list")(function* () {
    ...
    if (servers.length === 0) {
      ...
      prompts.outro("Add servers with: slopcode mcp add")
      return
    }
...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in slopcode.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
export const McpAuthListCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth.list")(function* () {
    ...
    for (const [name, serverConfig] of servers) {
      ...
      const icon = getAuthStatusIcon(authStatus)
      const statusText = getAuthStatusText(authStatus)
      const url = displayUrl(serverConfig.url)
...
export const McpLogoutCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.logout")(function* (args) {
    ...
    if (!serverName) {
      const selected = yield* Effect.promise(() =>
        prompts.select({
          ...
          options: servers.map(([name, server]) => ({ label: name, value: name, hint: displayUrl(server.url) })),
        }),
      )
      if (prompts.isCancel(selected)) throw new UI.CancelledError()
      serverName = selected
    ...
    if (!serverNames.includes(serverName)) {
      prompts.log.error(`No OAuth-capable MCP server found for: ${serverName}`)
      prompts.outro("Done")
      return
    }
...
async function resolveConfigPath(baseDir: string, global = false) {
  ...
  // Default to slopcode.json if none exist
  return candidates[0]
}
  ...
  handler: Effect.fn("Cli.mcp.add")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      if (args.name) {
        ...
        if (args.url && !validRemoteUrl(args.url)) {
          throw new Error("Invalid MCP URL: expected HTTP(S)")
        }
      ...
      if (type === "local") {
        const command = await prompts.text({
          ...
          placeholder: "e.g., slopcode x @modelcontextprotocol/server-filesystem",
          validate: (x) => (x && x.length > 0 ? undefined : "Required"),
        })
      ...
      if (type === "remote") {
        const url = await prompts.text({
          ...
          validate: (x) => {
            ...
            return validRemoteUrl(x) ? undefined : "Invalid MCP URL: expected HTTP(S)"
          },
        })
  ...
  handler: Effect.fn("Cli.mcp.debug")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      try {
        const response = await fetch(serverConfig.url, {
          ...
          body: JSON.stringify({
            ...
            params: {
              ...
              capabilities: {},
              clientInfo: { name: "slopcode-debug", version: InstallationVersion },
            },
            id: 1,
          }),
        })
        ...
        if (wwwAuth) {
          prompts.log.info(`WWW-Authenticate: ${safe(wwwAuth)}`)
        }
        ...
        if (response.status === 401) {
          ...
          const authProvider = new McpOAuthProvider(
            { instance: ctx.directory, name: serverName },
            serverConfig.url,
            {
          ...
          try {
            ...
            if (error instanceof UnauthorizedError) {
              prompts.log.info(`OAuth flow triggered: ${safe(error.message)}`)
              ...
              prompts.log.error(`Connection error: ${safe(error instanceof Error ? error.message : String(error))}`)
            }
          }
          ...
          try {
            ...
            if (json.result?.serverInfo) {
              prompts.log.info(`Server info: ${safe(JSON.stringify(json.result.serverInfo))}`)
            }
          } catch {
            // Not JSON, ignore
          }
          ...
          if (body) {
            prompts.log.info(`Response body: ${safe(body).substring(0, 500)}`)
          }
        }
      } catch (error) {
        spinner.stop("Connection failed", 1)
        prompts.log.error(`Error: ${safe(error instanceof Error ? error.message : String(error))}`)
      }
