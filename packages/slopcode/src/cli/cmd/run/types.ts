...
import type { SlopcodeClient, PermissionRequest, QuestionRequest, ToolPart } from "@slopcode-ai/sdk/v2"
import type { TuiConfig } from "@slopcode-ai/tui/config"
...
type PromptModel = Parameters<SlopcodeClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<SlopcodeClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<SlopcodeClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<SlopcodeClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<SlopcodeClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<SlopcodeClient["experimental"]["resource"]["list"]>>["data"]>
...
export type QuestionReject = Parameters<SlopcodeClient["question"]["reject"]>[0]
