...
//      slopcode server and runs interactive mode against it.
//
...
export const RunCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: SlopcodeClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: SlopcodeClient, sessionID: string) {
      ...
      async function current(sdk: SlopcodeClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: SlopcodeClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: SlopcodeClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: SlopcodeClient) {
      ...
      const sdk = createSlopcodeClient({
        baseUrl: "http://slopcode.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
