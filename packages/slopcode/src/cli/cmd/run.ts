...
import { createSlopcodeClient, type Message, type SlopcodeClient, type ToolPart } from "@slopcode-ai/sdk/v2"
...
export const RunCommand = cmd({
  ...
  handler: async (args) => {
    ...
    async function session(sdk: SlopcodeClient) {
    ...
    async function share(sdk: SlopcodeClient, sessionID: string) {
      ...
      if (!cfg.data) return
      if (cfg.data.share !== "auto" && !Flag.SLOPCODE_AUTO_SHARE && !args.share) return
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createSlopcodeClient({ baseUrl: "http://slopcode.internal", fetch: fetchFn })
      await execute(sdk)
    })
  },
})
