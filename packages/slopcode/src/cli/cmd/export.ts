...
function diff(kind: string, diffs: { file: string; before: string; after: string }[] | undefined) {
  return diffs?.map((item, i) => ({
    ...item,
    file: redact(`${kind}-file`, String(i), item.file),
    before: redact(`${kind}-before`, String(i), item.before),
    after: redact(`${kind}-after`, String(i), item.after),
  }))
}
    ...
    info: {
      ...
      revert: !data.info.revert
        ...
        : {
            ...
            diff:
              data.info.revert.diff === undefined ? undefined : redact("revert-diff", data.info.id, data.info.revert.diff),
          },
    },
