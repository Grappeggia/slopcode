...
type Input = {
  ...
  routes: RouteMap
  bump: () => void
  event: ReturnType<typeof useSDK>["event"]
...
function routeNavigate(route: ReturnType<typeof useRoute>, name: string, params?: Record<string, unknown>) {
  ...
  if (name === "session") {
    ...
    route.navigate({ type: "session", sessionID, workspaceID })
    return
  }

  route.navigate({ type: "plugin", id: name, data: params, workspaceID })
}
...
function stateApi(sync: ReturnType<typeof useSync>): TuiPluginApi["state"] {
  return {
    ...
    get vcs() {
      ...
      return { branch: sync.data.vcs.branch }
    },
    session: {
      count() {
    ...
    mcp() {
      return Object.entries(sync.data.mcp)
        ...
        .map(([name, raw]) => {
          const item = raw as { status: TuiSidebarMcpItem["status"]; error?: string }
          return {
            name,
            status: item.status,
  ...
  return {
    ...
    keybind: {
      match(key, evt: ParsedKey) {
        return input.keybind.match(key as never, evt)
      },
      print(key) {
        return input.keybind.print(key as never)
      },
    ...
    plugins: {
      ...
      async install() {
        return { ok: false, message: "plugins.install is only available in plugin context" }
      },
    },
    lifecycle,
    theme: {
