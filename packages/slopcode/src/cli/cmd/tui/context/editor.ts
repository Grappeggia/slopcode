...
export const { use: useEditorContext, provider: EditorContextProvider } = createSimpleContext({
  ...
  init: (props: { WebSocketImpl?: typeof WebSocket }) => {
    ...
    const connect = () => {
      ...
      current.addEventListener("open", () => {
        ...
        request("initialize", {
          ...
          capabilities: {},
          clientInfo: { name: "slopcode", version: "0.0.0" },
        })
      })
    ...
    const scheduleReconnect = () => {
      ...
      attempt += 1
      reconnect = setTimeout(connect, Math.min(1000 * 2 ** (attempt - 1), 10_000))
    }
...
function resolveEditorConnection(directory: string): EditorConnection | undefined {
  const port = parsePort(
    process.env.CLAUDE_CODE_SSE_PORT || process.env.SLOPCODE_EDITOR_SSE_PORT || process.env.OPENCODE_EDITOR_SSE_PORT,
  )
  if (port) {
    return {
...
function parseMessage(value: unknown) {
  ...
  try {
    const parsed = JsonRpcMessageSchema.safeParse(JSON.parse(value))
