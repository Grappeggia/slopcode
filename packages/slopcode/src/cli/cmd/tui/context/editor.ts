    ...
    onMount(() => {
      ...
      const connect = () => {
        ...
        current.addEventListener("close", () => {
          ...
          setStore("status", "connecting")
          attempt += 1
          scheduleReconnect(Math.min(1000 * 2 ** (attempt - 1), 30000))
        })
      }
...
function resolveEditorConnection(): EditorConnection | undefined {
  ...
  const port = parsePort(
    process.env.SLOPCODE_EDITOR_SSE_PORT || process.env.OPENCODE_EDITOR_SSE_PORT || process.env.CLAUDE_CODE_SSE_PORT,
  )
  if (!port) return
  return {
