...
import type { TuiPlugin, TuiPluginApi, TuiPluginModule, TuiPluginStatus } from "@slopcode-ai/plugin/tui"
...
function meta(item: TuiPluginStatus, width: number) {
  if (item.source === "internal") return width >= 120 ? "Built-in plugin" : "Built-in"
  ...
  return (
    <props.api.ui.DialogPrompt
      ...
      onConfirm={(raw) => {
        ...
        const mod = raw.trim()
        if (!mod) {
          props.api.ui.toast({ variant: "error", message: "Plugin package name is required" })
          return
        }
        ...
        void props.api.plugins
          ...
          .then((out) => {
            ...
            if (!out.tui) {
              props.api.ui.toast({ variant: "info", message: "Package has no TUI target to load in this app." })
              show(props.api)
              return
            }
            ...
            return props.api.plugins.add(mod).then((ok) => {
              ...
              props.api.ui.toast({ variant: "success", message: `Loaded ${mod} in current session.` })
              show(props.api)
            })
          })
...
function View(props: { api: TuiPluginApi }) {
  ...
  const flip = (x: string) => {
    ...
    void task
      .then((ok) => {
        if (!ok) props.api.ui.toast({ variant: "error", message: `Failed to update plugin ${item.id}` })
    ...
    <DialogSelect
      ...
      keybind={[
        ...
        },
        {
          ...
          keybind: add,
          disabled: lock(),
          onTrigger: () => showInstall(props.api),
        },
      ]}
      onSelect={(item) => {
        setCur(item.value)
