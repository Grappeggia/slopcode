...
export namespace Config {
  ...
  export function managedConfigDir() {
    return process.env.SLOPCODE_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
  }
  ...
  export const state = Instance.state(async () => {
    ...
    if (Flag.SLOPCODE_CONFIG) {
      result = mergeConfigConcatArrays(result, await loadFile(Flag.SLOPCODE_CONFIG))
      log.debug("loaded custom config", { path: Flag.SLOPCODE_CONFIG })
    }

    // Project config overrides global and remote config.
    if (!Flag.SLOPCODE_DISABLE_PROJECT_CONFIG) {
      for (const file of await ConfigPaths.projectFiles("slopcode", Instance.directory, Instance.worktree)) {
    ...
    if (process.env.SLOPCODE_CONFIG_CONTENT) {
      result = mergeConfigConcatArrays(
        result,
        await load(process.env.SLOPCODE_CONFIG_CONTENT, {
          dir: Instance.directory,
          source: "SLOPCODE_CONFIG_CONTENT",
        }),
      )
      log.debug("loaded custom config from SLOPCODE_CONFIG_CONTENT")
    }
  ...
  export async function needsInstall(dir: string) {
    ...
    if (depVersion.startsWith(aliasPrefix)) {
      ...
      if (aliasVersion === targetVersion) return false
    }
  ...
  async function loadCommand(dir: string) {
    ...
    for (const item of await Glob.scan("{command,commands}/**/*.md", {
      ...
      const patterns = ["/.slopcode/command/", "/.slopcode/commands/", "/command/", "/commands/"]
  ...
  async function loadAgent(dir: string) {
    ...
    for (const item of await Glob.scan("{agent,agents}/**/*.md", {
      ...
      const patterns = ["/.slopcode/agent/", "/.slopcode/agents/", "/agent/", "/agents/"]
      const file = rel(item, patterns) ?? path.basename(item)
   ...
   * 2. Local slopcode.json
   * 3. Global plugin/ directory
   * 4. Global slopcode.json
   *
  ...
  export const Server = z
    .object({
      ...
      mdnsDomain: z.string().optional().describe("Custom domain name for mDNS service (default: slopcode.local)"),
  ...
  async function load(text: string, options: { path: string } | { dir: string; source: string }) {
    ...
    const normalized = (() => {
      ...
      delete copy.tui
      log.warn("tui keys in slopcode config are deprecated; move them to tui.json", { path: source })
      return copy
    })()
