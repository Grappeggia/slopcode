...
import { InstallationLocal, InstallationVersion } from "@slopcode-ai/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@slopcode-ai/core/v1/config/console-state"
...
import { RemoteAuthError } from "@slopcode-ai/core/v1/config/error"
...
export class Service extends Context.Service<Service, Interface>()("@slopcode/Config") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://slopcode.dev/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.SLOPCODE_CONFIG && !Flag.SLOPCODE_CONFIG_DIR && !Flag.SLOPCODE_CONFIG_CONTENT) {
        const file = globalConfigFile()
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://slopcode.dev/config.json" }, null, 2))
            .pipe(Effect.catch(() => Effect.void))
        }
      }
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "config.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "slopcode.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "slopcode.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "SLOPCODE_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/slopcode`
        ...
        if (Flag.SLOPCODE_CONFIG) {
          yield* merge(Flag.SLOPCODE_CONFIG, yield* loadFile(Flag.SLOPCODE_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.SLOPCODE_CONFIG })
        }

        if (!Flag.SLOPCODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("slopcode", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.SLOPCODE_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from SLOPCODE_CONFIG_DIR", { path: Flag.SLOPCODE_CONFIG_DIR })
        }
        ...
        for (const dir of directories) {
          if (dir.endsWith(".slopcode") || dir === Flag.SLOPCODE_CONFIG_DIR) {
            for (const file of ["slopcode.json", "slopcode.jsonc"]) {
          ...
          // Auto-discovered plugins under `.slopcode/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.SLOPCODE_CONFIG_CONTENT) {
          const source = "SLOPCODE_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.SLOPCODE_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from SLOPCODE_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["SLOPCODE_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("SLOPCODE_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.SLOPCODE_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.SLOPCODE_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("SLOPCODE_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.SLOPCODE_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.SLOPCODE_DISABLE_PRUNE) {
