...
const TUI_SCHEMA_URL = "https://slopcode.dev/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from slopcode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const slopcode = await slopcodeFiles(input)
  for (const file of slopcode) {
...
async function slopcodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "slopcode"),
    ...(await Filesystem.findUp(["slopcode.json", "slopcode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "slopcode"))
  }
  if (Flag.SLOPCODE_CONFIG) files.push(Flag.SLOPCODE_CONFIG)
