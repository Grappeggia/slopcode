...
import { isRecord } from "@slopcode-ai/tui/util/record"
import { Global } from "@slopcode-ai/core/global"
import { FSUtil } from "@slopcode-ai/core/fs-util"
import { CurrentWorkingDirectory } from "./tui-cwd"
import { ConfigPlugin } from "@/config/plugin"
import { TuiKeybind } from "@slopcode-ai/tui/config/keybind"
...
import { TuiConfig } from "@slopcode-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@slopcode/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  // Every config dir we may read from: global config dir, any `.slopcode`
  // folders between cwd and home, and SLOPCODE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.SLOPCODE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit SLOPCODE_TUI_CONFIG override, if set.
  if (Flag.SLOPCODE_TUI_CONFIG) {
    const configFile = Flag.SLOPCODE_TUI_CONFIG
  ...
  // 4. `.slopcode` directories (and SLOPCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".slopcode") || dir === Flag.SLOPCODE_CONFIG_DIR)
