...
import { errorData, errorMessage } from "@slopcode-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@slopcode-ai/tui/context/theme"
...
import type { HostPluginApi, HostSlots } from "@slopcode-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@slopcode-ai/core/v1/config/plugin"
import { createCommandShim } from "@slopcode-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@slopcode-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".slopcode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".slopcode", "themes")
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.SLOPCODE_PURE ? [] : pluginOrigins
    if (Flag.SLOPCODE_PURE && pluginOrigins.length) {
    }
