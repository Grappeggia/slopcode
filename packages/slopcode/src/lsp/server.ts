...
export namespace LSPServer {
  ...
  export const Ty: Info = {
    ...
    async spawn(root) {
      if (!Flag.SLOPCODE_EXPERIMENTAL_LSP_TY) {
        return undefined
      }
    ...
    async spawn(root) {
      ...
      if (!bin) {
        ...
        const proc = Process.spawn(
          ["dotnet", "tool", "install", "roslyn-language-server", "--prerelease", "--tool-path", Global.Path.bin],
          {
            stdout: "pipe",
            stderr: "pipe",
            stdin: "pipe",
  ...
  export const JDTLS: Info = {
    ...
    async spawn(root) {
      ...
      const dataDir = await fs.mkdtemp(path.join(os.tmpdir(), "slopcode-jdtls-data"))
      return {
        process: spawn(
          java,
          [
