...
import { SlopCodeHttpApi } from "./api"
...
const QueryParameterSchemas: Record<string, OpenApiSchema> = {
  ...
  "GET /memory includeDisabled": QueryBooleanOpenApi,
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for slopcode serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(SlopCodeHttpApi)
...
export const PublicApi = SlopCodeHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "slopcode",
    version: "1.0.0",
