...
import { FrontmatterError } from "@slopcode-ai/core/v1/config/error"
import { ConfigMarkdown } from "@/config/markdown"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Glob } from "@slopcode-ai/core/util/glob"
...
// Built-in skill that ships with slopcode. The model's intuition for what an
// slopcode.json should look like is often wrong, and slopcode hard-fails on
// invalid config, so users hit cryptic startup errors. Loading this skill
// when the model is asked to touch slopcode's own config files gives it the
...
const CUSTOMIZE_SLOPCODE_SKILL_BODY = SkillPlugin.CustomizeSlopcodeContent
...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  for (const dir of configDirs) {
    yield* scan(state, dir, SLOPCODE_SKILL_PATTERN)
  }
...
export class Service extends Context.Service<Service, Interface>()("@slopcode/Skill") {}
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make(
      Effect.fn("Skill.state")(function* () {
        ...
        s.skills[CUSTOMIZE_SLOPCODE_SKILL_NAME] = {
