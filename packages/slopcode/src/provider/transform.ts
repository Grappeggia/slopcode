...
import type * as ModelsDev from "@slopcode-ai/core/models-dev"
...
export function options(input: {
  ...
  if (
    ...
    (input.model.providerID === "slopcode" && ["kimi-k2-thinking", "glm-4.6"].includes(input.model.api.id))
  ) {
...
export function smallOptions(model: Provider.Model) {
  ...
  if (model.providerID === "openrouter" || model.providerID === "llmgateway") {
    if (model.providerID === "openrouter" && small.reasoning?.effort === "low") {
      return model.api.id.includes("google") ? { thinking: { enabled: false } } : { reasoning: { effort: "none" } }
    }
