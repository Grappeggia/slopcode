// Slopcode publish boundary for core events. Attach routed instance location
...
import "@slopcode-ai/core/catalog"
import "@slopcode-ai/core/session/event"
import { Context, Effect, Layer } from "effect"

export class Service extends Context.Service<Service, EventV2.Interface>()("@slopcode/EventV2Bridge") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const unsubscribe = yield* events.listen((event) =>
      Effect.gen(function* () {
        if (event.type.startsWith("internal.") || !EventV2.isPublic(event)) return
        const ctx = yield* InstanceRef
