...
import type { Message, SlopcodeClient, SessionMessageResponse } from "@slopcode-ai/sdk/v2"
...
export const AuthMethodID = "slopcode-login"
...
export class Service extends Context.Service<Service, Interface>()("@slopcode/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `slopcode auth login` in the terminal",
      name: "Login with slopcode",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: SlopcodeClient) {
...
function makeUsageService(sdk: SlopcodeClient) {
...
async function loadDirectorySnapshot(sdk: SlopcodeClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, slopcode provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const slopcodeProvider = providers[ProviderV2.ID.make("slopcode")]
  const slopcodeModel = slopcodeProvider ? Provider.sort(Object.values(slopcodeProvider.models))[0] : undefined
  if (slopcodeProvider && slopcodeModel) return { providerID: slopcodeProvider.id, modelID: slopcodeModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "SlopCode service failure", service })
}
