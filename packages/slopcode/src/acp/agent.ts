...
import type { AssistantMessage, Event, SlopcodeClient, SessionMessageResponse, ToolPart } from "@slopcode-ai/sdk/v2"
...
export namespace ACP {
  ...
  export async function init({ sdk: _sdk }: { sdk: SlopcodeClient }) {
    return {
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // SlopCode stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    const slopcodeProvider = providers.find((p) => p.id === "slopcode")
    if (slopcodeProvider) {
      if (slopcodeProvider.models["big-pickle"]) {
        return { providerID: "slopcode", modelID: "big-pickle" }
      }
      const [best] = Provider.sort(Object.values(slopcodeProvider.models))
      if (best) {
        return {
