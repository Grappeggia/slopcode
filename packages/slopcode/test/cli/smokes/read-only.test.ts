...
describe("slopcode read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // SLOPCODE_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ slopcode }) =>
      Effect.gen(function* () {
        const r = yield* slopcode.spawn(["mcp", "list"])
        slopcode.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ slopcode }) =>
      Effect.gen(function* () {
        const r = yield* slopcode.spawn(["providers", "list"])
        slopcode.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ slopcode }) =>
      Effect.gen(function* () {
        const r = yield* slopcode.spawn(["models"])
        slopcode.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ slopcode }) =>
      Effect.gen(function* () {
        const r = yield* slopcode.spawn(["agent", "list"])
        slopcode.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh SLOPCODE_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ slopcode }) =>
      Effect.gen(function* () {
        const r = yield* slopcode.spawn(["session", "list"])
        slopcode.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ slopcode }) =>
      Effect.gen(function* () {
        const r = yield* slopcode.spawn(["stats"])
        slopcode.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under SLOPCODE_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ slopcode }) =>
      Effect.gen(function* () {
        const r = yield* slopcode.spawn(["db", "path"])
        slopcode.expectExit(r, 0, "db path")
