...
describe("slopcode acp initialize/auth subprocess", () => {
  cliIt.live(
    ...
    ({ slopcode }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.agentInfo?.name).toBe("SlopCode")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ slopcode }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ slopcode })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("slopcode-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "slopcode-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.SLOPCODE_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ slopcode }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("slopcode-login")
