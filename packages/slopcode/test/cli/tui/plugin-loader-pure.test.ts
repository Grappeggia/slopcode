...
`,
  ...
  const pure = process.env.SLOPCODE_PURE
  const meta = process.env.SLOPCODE_PLUGIN_META_FILE
  process.env.SLOPCODE_PURE = "1"
  process.env.SLOPCODE_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.SLOPCODE_PURE
    } else {
      process.env.SLOPCODE_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.SLOPCODE_PLUGIN_META_FILE
    } else {
      process.env.SLOPCODE_PLUGIN_META_FILE = meta
    }
  }
})
