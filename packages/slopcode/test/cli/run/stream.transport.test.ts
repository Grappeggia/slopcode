...
type EventStream = Awaited<ReturnType<SlopcodeClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<SlopcodeClient["global"]["event"]>>["stream"]
...
type SessionChild = NonNullable<Awaited<ReturnType<SlopcodeClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<SlopcodeClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://slopcode.test"),
...
function sdk(
  input: {
    ...
    subscribe?: SlopcodeClient["event"]["subscribe"]
    globalEvent?: SlopcodeClient["global"]["event"]
    promptAsync?: SlopcodeClient["session"]["promptAsync"]
    status?: SlopcodeClient["session"]["status"]
    messages?: SlopcodeClient["session"]["messages"]
    children?: SlopcodeClient["session"]["children"]
    permissions?: SlopcodeClient["permission"]["list"]
    questions?: SlopcodeClient["question"]["list"]
  } = {},
) {
  const client = new SlopcodeClient()

  const subscribe: SlopcodeClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: SlopcodeClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: SlopcodeClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: SlopcodeClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: SlopcodeClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: SlopcodeClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: SlopcodeClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: SlopcodeClient["question"]["list"] = input.questions ?? (() => ok([]))
