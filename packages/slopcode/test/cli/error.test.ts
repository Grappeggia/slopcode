...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/slopcode.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/slopcode.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/slopcode.jsonc", dir: ".slopcode", suggestion: "slopcode" },
        expected:
          'Directory ".slopcode" in /tmp/slopcode.jsonc is not valid. Rename the directory to "slopcode" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        data: {
          path: "/tmp/slopcode.jsonc",
          message: "schema mismatch",
          issues: [{ message: "Expected string", path: ["provider", "id"] }],
        },
        expected: "Configuration is invalid at /tmp/slopcode.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Try: `slopcode models` to list available models",
      "Or check your config (slopcode.json) provider/model names",
    ].join("\n")
