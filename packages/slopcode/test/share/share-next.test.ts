...
import { SessionShareTable } from "@slopcode-ai/core/share/sql"
    ...
    provideTmpdirInstance(() =>
      ShareNext.Service.use((svc) =>
        Effect.gen(function* () {
          ...
          expect(req.baseUrl).toBe("https://slopcode.dev")
          expect(req.api.create).toBe("/api/share")
          expect(req.headers).toEqual({})
        }),
      ).pipe(Effect.provide(requestLayer(none))),
    ...
    provideTmpdirInstance(
      () => {
        ...
        return Effect.gen(function* () {
          ...
          const created = seen.filter((req) => req.url === "https://legacy-share.example.com/api/share")
    ...
    provideTmpdirInstance(
      () => {
        ...
        return Effect.gen(function* () {
          ...
          expect(
            seen.filter((req) => req.method === "POST" && req.url === "https://legacy-share.example.com/api/share"),
          ).toHaveLength(1)
          expect(
            seen.filter(
