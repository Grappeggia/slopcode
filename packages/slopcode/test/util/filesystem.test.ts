...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "slopcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "slopcode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "slopcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "slopcode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["slopcode.json", "slopcode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "slopcode.json"),
        path.join(project, "slopcode.jsonc"),
      ])
    })
  })
