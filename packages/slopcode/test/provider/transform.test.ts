...
describe("ProviderTransform.options - gpt-5 textVerbosity", () => {
  ...
  test("openai gpt-5 models keep reasoningSummary", () => {
    const result = ProviderTransform.options({
      model: createGpt5Model("gpt-5.4"),
      sessionID,
      providerOptions: {},
    })
  ...
  test("azure completion URLs omit Responses-only reasoning options after merge", () => {
    const model = {
      ...createGpt5Model("gpt-5.4"),
      id: "azure/gpt-5.4",
      providerID: "azure",
    ...
    const result = ProviderTransform.requestOptions({
      model,
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    ...
    const result = ProviderTransform.message(msgs, slopcodeModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.slopcode?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.slopcode?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              slopcode: { itemId: "msg_slopcode_part" },
    ...
    expect(result[0].providerOptions?.slopcode?.itemId).toBe("msg_slopcode")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.slopcode?.itemId).toBe("msg_slopcode_part")
