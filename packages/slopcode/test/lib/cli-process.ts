// Subprocess test harness for the slopcode CLI. Spawns the real binary against
...
//
// Configuration flows through slopcode's built-in test affordances:
//   - SLOPCODE_CONFIG_CONTENT      : provider config inline, no files to find
//   - SLOPCODE_TEST_HOME           : pins os.homedir() → tmpdir
//   - SLOPCODE_DISABLE_PROJECT_CONFIG : skip walking up for slopcode.json
//   - SLOPCODE_PURE                : skip external plugin discovery + install
//   - SLOPCODE_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `slopcode.run` is fully wired. The shape supports adding more
// builders (`slopcode.serve(opts)`, `slopcode.acp(opts)`, `slopcode.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of SlopcodeCli.
...
const slopcodeRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(slopcodeRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    ...
    SLOPCODE_CONFIG_CONTENT: configJson,
    SLOPCODE_DISABLE_PROJECT_CONFIG: "1",
    SLOPCODE_PURE: "1",
    SLOPCODE_DISABLE_AUTOUPDATE: "1",
    SLOPCODE_DISABLE_AUTOCOMPACT: "1",
    SLOPCODE_DISABLE_MODELS_FETCH: "1",
    SLOPCODE_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `slopcode run`. New flags should
...
// `slopcode serve` is a long-lived process — it never exits on its own.
...
// `slopcode acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type SlopcodeCli = {
  // High-level: run a single prompt against the test model. Short-lived.
  readonly run: (message: string, opts?: RunOpts) => Effect.Effect<RunResult>
  // Spawn `slopcode serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `slopcode acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("slopcode.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const serve = Effect.fn("slopcode.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "slopcode server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `slopcode serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("slopcode.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const slopcode: SlopcodeCli = { run, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, slopcode })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `slopcode.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
...
function expectExit(result: RunResult, expected: number, label = "slopcode") {
...
// every call site. Use this for any test that needs the slopcode CLI fixture.
//
...
// resources (e.g. `slopcode.serve`) without an extra `Effect.scoped` wrapper —
