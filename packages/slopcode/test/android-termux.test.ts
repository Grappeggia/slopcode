...
describe("Android Termux runtime", () => {
  ...
  test("Android runtime stays Rust-only while root package ships the bootstrap bundle", async () => {
    ...
    expect(nativeTest).toContain(
      "drives terminal polish panels and title updates through Android native handlers",
    )
    ...
    expect(nativeTest).toContain("drives editor open, dirty guard, diagnostics, save, and diff dismissal through Android native routes")
