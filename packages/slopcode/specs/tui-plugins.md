...
- Author package entrypoint is `@slopcode-ai/plugin/tui`.
...
```json
{
  ...
  "plugin": ["@acme/slopcode-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@slopcode-ai/plugin/tui`.
- `@slopcode-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.
...
- The value is a semver range checked against the running SlopCode version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.slopcode` is absent, no check is performed (backward compatible).
...
- `slopcode plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `slopcode plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.slopcode` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.slopcode`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.slopcode`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.slopcode` dir or `SLOPCODE_CONFIG_DIR`, SlopCode installs `@slopcode-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@slopcode-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus SlopCode's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
SlopCode registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, SlopCode pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@slopcode-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"slopcode"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `slopcode.default`.
...
- Local plugins persist installed themes under the local `.slopcode/themes` area near the plugin config source.
...
- `--pure` / `SLOPCODE_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.slopcode/plugins/tui-smoke.tsx`
- Local vim plugin: `.slopcode/plugins/tui-vim.tsx`
- Local smoke config: `.slopcode/tui.json`
- Local smoke theme: `.slopcode/plugins/smoke-theme.json`
