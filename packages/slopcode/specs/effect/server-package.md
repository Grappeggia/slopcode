...
Practical reference for a future `packages/server` split after the slopcode
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/slopcode`.
...
Until enough shared service code lives outside `packages/slopcode`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/slopcode`

It should not import `packages/slopcode` services while `packages/slopcode`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on slopcode-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/slopcode` implementation details.
