...
Delete `packages/slopcode/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/slopcode/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/slopcode/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/slopcode`
- TUI plugin loader tests in `packages/slopcode` if runtime plugin API wiring changed
