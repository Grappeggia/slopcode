...
const binaries = await Array.fromAsync(new Bun.Glob("*/package.json").scan({ cwd: "./dist" })).then((arr) =>
  Promise.all(
    arr.map(async (filepath) => {
      ...
      const item = (await Bun.file(`./dist/${filepath}`).json()) as { name: string; version: string }
      const name = item.name.startsWith(`${pkg.name}-bin-bin-`)
        ? item.name.replace(`${pkg.name}-bin-bin-`, `${pkg.name}-bin-`)
        : item.name.startsWith(`${pkg.name}-`) && !item.name.startsWith(`${pkg.name}-bin-`)
          ? item.name.replace(`${pkg.name}-`, `${pkg.name}-bin-`)
          : item.name
...
const verifyAptParity = async () => {
  if (!latestRelease || !enforceApt || dry) {
    return
  }
...
const verifyRpmParity = async () => {
  if (!latestRelease || !enforceRpm || dry) {
    return
  }
  const target = Script.version.replace(/^v/, "")
  const loop = async (left: number) => {
...
const verifyApkParity = async () => {
  if (!latestRelease || !enforceApk || dry) {
    return
  }
  const target = Script.version.replace(/^v/, "")
  const loop = async (left: number) => {
...
const publishBinary = async (binary: (typeof binaries)[number]) => {
  ...
  if (!skipPack || dry) {
    await pack(`./dist/${binary.dir}`, binary.name)
  }

  if (dry) {
    console.log("dry run", binary.name, binary.version)
    return
  }
...
if (Script.channel === "latest" && supplemental && !dry) {
  ...
  console.log(dry ? "supplemental channels: skipped dry run" : "supplemental channels: disabled")
}
