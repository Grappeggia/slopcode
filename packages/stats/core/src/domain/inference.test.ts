...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve slopcode route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "slopcode")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "slopcode")).toBe("openai")
    expect(statProvider("big-pickle", "", "slopcode")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "slopcode"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep slopcode as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "slopcode"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "slopcode"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep slopcode or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "slopcode"), country: "US" })).toMatchObject([
