...
import {
  ...
  type StatsModelData,
  type UsageRange,
} from "@slopcode-ai/stats-core/domain/home"
import { runtime } from "@slopcode-ai/stats-core/runtime"
...
const statsCanonicalBaseUrl = "https://slopcode.ai/data/"
const statsUnfurlPath = "banner.png"
const statsUnfurlAlt = "SlopCode Data wordmark on a dark patterned background"
...
export default function StatsModel() {
  ...
  const modelDescription = createMemo(() =>
    stats()
      ? `${modelName()} usage, rank, token mix, cost, geo breakdown, and peer data across SlopCode.`
      : `${modelName()} model facts, limits, and SlopCode usage availability.`,
  )
      ...
      <div data-slot="model-hero-grid">
        <div data-slot="model-hero-copy">
          ...
          <Show
            when={props.data}
            fallback={
              <p>Model facts from the shared model index. SlopCode usage appears once this model has activity.</p>
            }
          >
            {(data) => (
              <p>
                Ranked #{data().rank} across recent SlopCode token usage with {formatPercent(data().tokenShare)} of
                observed volume.
              </p>
            )}
          </Show>
...
function ModelCatalogCallout(props: { catalog: ModelCatalogEntry | null }) {
  return (
    <div data-component="model-rank-panel">
      ...
      <p>No SlopCode usage in the current data window.</p>
    </div>
  )
}
...
function ModelOverview(props: { data: StatsModelData | null }) {
  return (
    <section data-section="model-panel">
      ...
      <Show
        when={props.data}
        fallback={<ModelEmptyState title="No usage summary" description="This model has no SlopCode usage rows yet." />}
      >
        {(data) => (
