...
import { createDraftTab } from "./tab-state"
import type { SessionTab, Tab } from "./tabs"
import { reorderTabs } from "./tab-state"
...
describe("tab migration", () => {
  ...
  test("preserves a valid draft model and drops malformed model state", () => {
    const draft = {
      type: "draft",
      draftID: "draft",
      server,
      directory: "/tmp",
      model: { providerID: "slopcode", modelID: "big-pickle", variant: "high" },
    } as const

    expect(migrateTabs([draft], server)).toEqual([draft])
    const invalid = migrateTabs([{ ...draft, model: { providerID: "slopcode" } }], server)[0]
    expect(invalid?.type === "draft" ? invalid.model : undefined).toBeUndefined()
  })
})

test("new draft state clones the selected model for carryover", () => {
  const model = { providerID: "slopcode", modelID: "big-pickle", variant: "high" }
  const draft = createDraftTab("draft", { server, directory: "/tmp" }, model)

  model.modelID = "changed"
  expect(draft.model).toEqual({ providerID: "slopcode", modelID: "big-pickle", variant: "high" })
})
...
describe("closed tab stack", () => {
  ...
  test("treats a draft with extra query state as active when closing", () => {
    const draft: Tab = { type: "draft", draftID: "draft", server, directory: "/tmp" }
    const controller = createTabController({
      activeServer: () => server,
      servers: () => [server],
      location: () => ({ pathname: "/new-session", search: "?draftId=draft&prompt=hello" }),
    })

    expect(controller.remove([draft], 0).next).toBeNull()
  })
  ...
  test("close and reopen follow the persisted reordered tab positions", () => {
    const controller = createTabController({
      activeServer: () => server,
      servers: () => [server],
      location: () => ({ pathname: tabHref(sessionTab("a")), search: "" }),
    })
    const tabs = [sessionTab("a"), sessionTab("b"), sessionTab("c")]
    const reordered = reorderTabs(tabs, [tabKey(tabs[2]), tabKey(tabs[0]), tabKey(tabs[1])], tabKey)
    const closed = controller.close(reordered, [], 1)

    expect(closed.next).toEqual(tabs[1])
    expect(closed.tabs).toEqual([tabs[2], tabs[1]])
    expect(controller.reopen(closed.tabs, closed.closed).tabs).toEqual(reordered)
  })

  test("closes and reopens a parent tab while its child session route is selected", () => {
    const parent = sessionTab("parent")
    const other = sessionTab("other")
    const controller = createTabController({
      activeServer: () => server,
      servers: () => [server],
      location: () => ({ pathname: tabHref({ ...parent, sessionId: "child" }), search: "" }),
    })

    const closed = controller.close([parent, other], [], 0, true)
    expect(closed.next).toEqual(other)
    expect(closed.tabs).toEqual([other])
    expect(controller.reopen(closed.tabs, closed.closed).tabs).toEqual([parent, other])
  })
