  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://slopcode.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://slopcode.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://slopcode.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://slopcode.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://slopcode.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://slopcode.example.com"),
      ),
    ).toEqual({
