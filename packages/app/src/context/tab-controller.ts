...
export function createTabController(input: {
  ...
  const remove = (tabs: Tab[], index: number) => ({
    tab: tabs[index],
    tabs: tabs.filter((_, current) => current !== index),
    next: nextTabAfterClose(tabs, index, !!tabs[index] && active(tabs[index])),
  })

  const close = (tabs: Tab[], closed: ClosedTab[], index: number) => {
    const result = remove(tabs, index)
    return {
      ...result,
