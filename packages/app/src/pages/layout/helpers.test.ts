...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("slopcode://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("slopcode://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("slopcode://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("slopcode://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("slopcode://open-project")).toBeUndefined()
    expect(parseDeepLink("slopcode://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "slopcode://open-project?directory=/a",
      "slopcode://other?directory=/b",
      "slopcode://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("slopcode://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("slopcode://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("slopcode://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("slopcode://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "slopcode://new-session?directory=/a",
      "slopcode://open-project?directory=/b",
      "slopcode://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __SLOPCODE__: {
        deepLinks: ["slopcode://open-project?directory=/a"],
      },
    } as unknown as Window & { __SLOPCODE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["slopcode://open-project?directory=/a"])
