...
export type WslJob =
  ...
  | { kind: "probe-slopcode"; distro: string; startedAt: number }
  | { kind: "install-slopcode"; distro: string; startedAt: number }
...
export type WslServersPlatform = {
  ...
  probeSlopcode(name: string): Promise<void>
  installSlopcode(name: string): Promise<void>
