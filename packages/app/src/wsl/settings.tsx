...
import { Tag } from "@slopcode-ai/ui/v2/badge-v2"
import { ButtonV2 } from "@slopcode-ai/ui/v2/button-v2"
import { Dialog } from "@slopcode-ai/ui/v2/dialog-v2"
import { Icon as IconV2 } from "@slopcode-ai/ui/v2/icon"
import { IconButtonV2 } from "@slopcode-ai/ui/v2/icon-button-v2"
import { MenuV2 } from "@slopcode-ai/ui/v2/menu-v2"
...
import { wslSlopcodeAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.slopcodeChecks[item.config.distro]
          const slopcodeAction = () => wslSlopcodeAction(check())
          const busy = () => wsl.data?.job?.kind === "install-slopcode" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={slopcodeAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installSlopcode(item.config.distro))}
                    >
