...
import { enterWslSlopcodeStep } from "./settings-model"

type WslServerStep = "wsl" | "distro" | "slopcode"

const STEPS: WslServerStep[] = ["wsl", "distro", "slopcode"]
...
export function DialogAddWslServer(props: DialogWslServerProps = {}) {
  ...
  const slopcodeCheck = createMemo(() => {
    const distro = selectedDistro()
    if (!distro) return null
    return current()?.slopcodeChecks[distro] ?? null
  })
  ...
  const slopcodeReady = createMemo(() => {
    const check = slopcodeCheck()
  ...
  const installingSlopcode = createMemo(() => {
    const job = current()?.job
    return job?.kind === "install-slopcode" && job.distro === selectedDistro()
  })
  const allReady = createMemo(() => wslReady() && distroReady() && slopcodeReady())
  const addDisabled = createMemo(() => {
    const job = current()?.job
    if (!job) return store.adding
    return store.adding || job.kind !== "probe-slopcode"
  })
  ...
  const autoProbe = createMemo(() => {
    ...
    if (!state.slopcodeChecks[distro]) {
      return { key: `probe-slopcode:${distro}`, run: () => api.probeSlopcode(distro) }
    }
    return null
  })
  ...
  const slopcodeMessage = createMemo(() => {
    const state = current()
    if (!state) return language.t("wsl.onboarding.checkingSlopcode")
    const distro = selectedDistro()
    if (state.job?.kind === "install-slopcode") {
      return distro
        ? language.t("wsl.onboarding.updatingSlopcodeIn", { distro })
        : language.t("wsl.onboarding.updatingSlopcode")
    }
    if (state.job?.kind === "probe-slopcode") {
      return distro
        ? language.t("wsl.onboarding.checkingSlopcodeIn", { distro })
        : language.t("wsl.onboarding.checkingSlopcode")
    }
    if (slopcodeCheck()?.error) return slopcodeCheck()!.error
    if (slopcodeCheck()?.matchesDesktop === false) {
      return distro
        ? language.t("wsl.onboarding.updateSlopcodeIn", { distro })
        : language.t("wsl.onboarding.updateSlopcode")
    }
    if (slopcodeReady()) {
      return distro
        ? language.t("wsl.onboarding.slopcodeReadyIn", { distro })
        : language.t("wsl.onboarding.slopcodeReady")
    }
    return distro
      ? language.t("wsl.onboarding.installSlopcodeIn", { distro })
  ...
  const openSlopcodeStep = () => {
    const distro = selectedDistro()
    if (!distro) return
    void run(() => enterWslSlopcodeStep(distro, api.probeSlopcode, (step) => setStore("step", step)))
  }
        ...
        title:
          step === "wsl"
            ...
            : step === "distro"
              ...
              : language.t("wsl.onboarding.step.slopcode"),
        state:
          active === step
            ? "current"
            : step === "wsl"
              ...
              : step === "distro"
                ? distroReady()
                  ...
                  : index > activeIndex
                    ? "locked"
                    : "warning"
                : slopcodeCheck()?.matchesDesktop === false
                  ? "warning"
                  : slopcodeReady()
                    ? "done"
                    : index > activeIndex
      ...
      >
        ...
        >
          ...
          <Switch>
            ...
            <Match when={activeStep() === "slopcode"}>
              <div class="rounded-md bg-surface-base p-4 flex flex-col gap-3">
                <div class="flex items-center justify-between gap-3">
                  <div class="text-14-medium text-text-strong">{language.t("wsl.onboarding.step.slopcode")}</div>
                  <div class="flex items-center gap-2">
                    <Show when={selectedDistro()}>
                      <Button
                        variant="ghost"
                        size="large"
                        disabled={busy()}
                        onClick={() => runSelectedDistro((distro) => api.probeSlopcode(distro))}
                      >
                        {language.t("wsl.onboarding.refresh")}
                      </Button>
                    </Show>
                    <Show when={!slopcodeReady() || slopcodeCheck()?.matchesDesktop === false}>
                      <Button
                        variant="secondary"
                        size="large"
                        disabled={busy()}
                        onClick={() => runSelectedDistro((distro) => api.installSlopcode(distro))}
                      >
                        <Show when={installingSlopcode()}>
                          <Spinner class="size-4 shrink-0" />
                        </Show>
                        {slopcodeCheck()?.resolvedPath
                          ? language.t("wsl.onboarding.updateSlopcode")
                          : language.t("wsl.onboarding.installSlopcode")}
                      </Button>
                    </Show>
                  </div>
                </div>
                <div class="text-12-regular text-text-weak whitespace-pre-wrap break-words">{slopcodeMessage()}</div>
                <Show when={slopcodeCheck()?.matchesDesktop === false ? slopcodeCheck() : null}>
                  {(check) => (
                    <div class="rounded-md border border-border-weak-base px-3 py-3 flex flex-col gap-1">
                      <div class="text-12-regular text-text-weak">
          ...
          <Show when={activeStep() === "slopcode" && allReady() && selectedDistro()}>
