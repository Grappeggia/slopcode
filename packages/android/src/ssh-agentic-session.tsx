...
export function SshAgenticSession(props: Props) {
  ...
  const start = async () => {
    ...
    try {
      ...
      const workspaceObject = workspaceValue && typeof workspaceValue === "object" && !Array.isArray(workspaceValue) ? workspaceValue as { id?: unknown } : undefined
      const workspaceID =
        workspaceObject && typeof workspaceObject.id === "string"
          ? workspaceObject.id
          : ""
      ...
      <section class="w-full max-w-3xl rounded-2xl border border-border-weak-base bg-surface-raised-base p-4 sm:p-6 flex flex-col gap-5">
        <header class="flex items-start justify-between gap-4">
          <div class="flex flex-col gap-1">
            ...
            <p class="text-14-regular text-text-weak">Ask for an outcome. Slopcode will plan, act, and show you what changed.</p>
          </div>
          <button type="button" onClick={() => void disconnect()} class="shrink-0 rounded-md border border-border-weak-base px-3 py-2 text-12-regular">
            Disconnect
          </button>
        </header>
        ...
        <section class="rounded-xl border border-border-weak-base bg-surface-base p-4" aria-label="Connection progress">
          <div class="flex items-center justify-between gap-3">
            ...
            <Show when={busy()}>
              <span class="text-12-regular text-text-weak" aria-live="polite">Working…</span>
            </Show>
          </div>
            ...
            <For each={["Connecting", "Workspace", "Agent", "Session"]}>
              {(label, index) => (
                <div class="flex flex-col gap-2" aria-label={label}>
                  <div class={`h-2 rounded-full ${index() <= (state().phase === "connecting" ? 0 : state().phase === "opening" ? 1 : state().phase === "ready" ? 2 : 3) ? "bg-surface-brand-base" : "bg-surface-weak-base"}`} />
        ...
        <Show when={error() || state().error}>
          <div role="alert" class="rounded-xl border border-border-critical-base bg-surface-critical-weak p-4 flex flex-col gap-3">
            <p class="text-14-regular">{error() || state().error}</p>
            <div class="flex flex-wrap gap-2">
              <button type="button" onClick={() => void reconnect()} class="rounded-md bg-surface-brand-base text-text-on-brand-base px-3 py-2 text-12-medium">Reconnect</button>
              <Show when={state().lastPrompt}>
                <button type="button" onClick={retry} class="rounded-md border border-border-weak-base px-3 py-2 text-12-medium">Retry last request</button>
              </Show>
            </div>
          </div>
        </Show>
              ...
              {(item) => (
                <article class="rounded-xl border border-border-weak-base bg-surface-base p-4 flex flex-col gap-2">
                  ...
                  <Show when={item.type === "output" || item.type === "reasoning" || item.type === "retry"}>
                    <p class="text-14-regular whitespace-pre-wrap break-words">{item.type === "output" || item.type === "reasoning" || item.type === "retry" ? item.text : ""}</p>
                  </Show>
                  ...
                  <Show when={item.type === "artifact"}>
                    ...
                    <p class="text-12-regular text-text-weak break-all">{item.type === "artifact" ? item.path : ""}</p>
                  </Show>
                </article>
              )}
            </For>
        ...
        <Show when={state().interaction}>
          {(interaction) => (
            <section class="rounded-xl border border-border-brand-base bg-surface-base p-4 flex flex-col gap-3" aria-label={interaction().kind === "approval" ? "Approval required" : "Question from agent"}>
              <div>
                <p class="text-12-regular text-text-weak">{interaction().kind === "approval" ? "Approval required" : "Question"}</p>
                <h2 class="text-16-medium mt-1">{interaction().title ?? interaction().prompt}</h2>
              </div>
              <Show when={interaction().command}>
                <pre class="rounded-lg bg-surface-raised-base p-3 text-12-regular whitespace-pre-wrap break-words">{interaction().command}</pre>
              </Show>
              <Show when={interaction().reason || interaction().cwd}>
                <p class="text-12-regular text-text-weak">{interaction().reason ?? ""}{interaction().cwd ? ` · ${interaction().cwd}` : ""}</p>
              </Show>
              <Show when={interaction().kind === "approval"}>
                <div class="flex flex-wrap gap-2">
                  <button type="button" disabled={busy()} onClick={() => void respond(interaction(), "approved")} class="rounded-md bg-surface-brand-base text-text-on-brand-base px-4 py-2 text-12-medium disabled:opacity-50">Approve</button>
                  <button type="button" disabled={busy()} onClick={() => void respond(interaction(), "rejected")} class="rounded-md border border-border-weak-base px-4 py-2 text-12-medium disabled:opacity-50">Reject</button>
                </div>
              </Show>
                ...
                <Show when={interaction().options?.length}>
                  <div class="flex flex-wrap gap-2">
                    <For each={interaction().options ?? []}>
                      {(option) => <button type="button" disabled={busy()} onClick={() => choose(option)} class="rounded-md border border-border-weak-base px-3 py-2 text-12-medium disabled:opacity-50">{option}</button>}
                    </For>
                  </div>
                </Show>
                <Show when={interaction().allowFreeform !== false}>
                  <div class="flex gap-2">
                    <input value={answer()} onInput={(event) => setAnswer(event.currentTarget.value)} placeholder="Your answer" class="min-w-0 flex-1 rounded-md border border-border-weak-base bg-surface-raised-base px-3 py-2 text-14-regular" />
                    <button type="button" disabled={busy() || !answer().trim()} onClick={() => void respond(interaction())} class="rounded-md bg-surface-brand-base text-text-on-brand-base px-3 py-2 text-12-medium disabled:opacity-50">Send</button>
                  </div>
                </Show>
              </Show>
            </section>
        ...
        <form class="flex flex-col gap-2" onSubmit={(event) => { event.preventDefault(); void send() }}>
          <label class="text-14-medium" for="agent-prompt">What should the agent do?</label>
          <div class="flex gap-2">
            <textarea id="agent-prompt" rows="3" value={prompt()} onInput={(event) => setPrompt(event.currentTarget.value)} placeholder="e.g. Review the latest changes and summarize any risks" disabled={busy() || state().phase === "waiting"} class="min-w-0 flex-1 resize-y rounded-xl border border-border-weak-base bg-surface-base px-3 py-3 text-14-regular disabled:opacity-50" />
            <button type="submit" disabled={busy() || !prompt().trim() || !state().sessionID || state().phase === "waiting"} class="self-end rounded-md bg-surface-brand-base text-text-on-brand-base px-4 py-3 text-12-medium disabled:opacity-50">Send</button>
          </div>
        </form>

        <div class="flex flex-wrap items-center justify-between gap-2">
          <button type="button" disabled={busy() || state().phase !== "running"} onClick={() => void stop()} class="rounded-md border border-border-weak-base px-3 py-2 text-12-medium disabled:opacity-50">Stop</button>
