        ...
        <section class="mx-auto flex w-full max-w-2xl flex-col gap-5 rounded-2xl border border-border-weak-base bg-surface-raised-base p-4 sm:p-6">
          <header class="flex flex-col gap-2">
            ...
            <p class="text-14-regular text-text-weak">
              This saved job remains available for recovery. It is not an SSH PTY session, so the app will not pretend
              to resume it through the native terminal.
            </p>
          </header>
          ...
          <section class="grid grid-cols-1 gap-3 sm:grid-cols-2" aria-label="Durable remote job details">
            <div class="rounded-xl border border-border-weak-base bg-surface-base p-3">
              ...
              <p data-ssh-durable-job-status class="mt-1 text-14-medium">
                {remoteJobStatusLabel(props.job.status)}
              </p>
            </div>
            <div class="rounded-xl border border-border-weak-base bg-surface-base p-3">
              <p class="text-12-regular text-text-weak">Session</p>
              <p data-ssh-durable-job-session class="mt-1 break-all text-14-medium">
                {props.job.sessionID}
              </p>
            </div>
          ...
          <Show when={props.job.error}>
            <p
              role="alert"
              class="rounded-xl border border-border-critical-base bg-surface-critical-weak p-3 text-14-regular"
            >
              {props.job.error}
            </p>
          </Show>
