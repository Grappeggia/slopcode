...
describe("SSH shell accessibility regressions", () => {
  ...
  test("exposes the message-first agent DOM, live semantics, persistent labels, focus, and honest review tabs", () => {
    ...
    expect(agentic).toContain('data-agent-status-live role="status" aria-live="polite"')
    expect(agentic).not.toContain('aria-label="Conversation" aria-live=')
    expect(agentic).toContain("data-agent-new-activity")
    expect(agentic).toContain('aria-label="Jump to new activity"')
