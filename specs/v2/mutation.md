...
The explicit platform adapter advertises mutation, staging, and exchange capabilities independently of platform names. Windows currently advertises none: Core has no reviewed NT handle-relative helper, so every primitive fails closed with `FileMutation.UnsupportedPlatformError`. There is no pathname fallback and no `unsupported-security` formatter outcome. A Windows distribution may enable mutation only by supplying a separately reviewed native adapter that preserves no-reparse parent handles, verified final paths, handle-relative create/delete, and identity-safe commit semantics.
...
5. For an internal target, creates an unpredictable exclusive `0600` hidden stage beside the target through its stable verified directory handle. For an external target, creates that same basename/extension-preserving stage under the verified active Location root. Linux passes the owner-process descriptor path. Darwin passes the cryptographically unpredictable canonical child path, keeps the no-follow parent and stage handles open, and verifies parent and stage identities before and after child execution. It copies immediate bytes and runs every matching formatter sequentially there; deletes and primitive no-ops skip formatting.
...
Delete tracks the identity and current role of every generated quarantine and placeholder until scope completion. Deterministic barriers cover pre-exchange, post-exchange, placeholder movement, and rollback. A mismatch is restored atomically by exchange or exclusive rename. Every failure after exchange, including approved-inode unlink, rollback exchange, rollback-placeholder unlink, public-placeholder move, replacement restore, inspection, and final placeholder unlink, returns `FileMutation.RecoveryConflictError` with a machine-readable state and all canonical recovery paths. It never unlinks the conflicting public entry or displaced replacement. Scoped finalizers unlink only a name whose current identity is the generated placeholder and restore the approved inode only when both identities still match.
