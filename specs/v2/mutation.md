...
Delete tracks the identity and current role of every generated quarantine and placeholder until scope completion. Deterministic barriers cover pre-exchange, post-exchange, placeholder movement, and rollback. A mismatch is restored atomically by exchange or exclusive rename. If a concurrent public entry prevents restoration, the operation returns `FileMutation.RecoveryConflictError` with the canonical recovery path; it never unlinks the conflicting public entry or the displaced replacement. Scoped finalizers unlink only a name whose current identity is the generated placeholder and restore the approved inode only when both identities still match.
