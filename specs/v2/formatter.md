...
Execution uses direct `ChildProcess` argv with no implicit shell. During post-mutation settlement, every literal `$FILE` in every argument becomes a private hidden same-basename-derived, same-extension staging file beside the approved target, never the live pathname; a target is not appended when no placeholder exists. Linux exposes the verified parent handle as `/proc/<owner-pid>/fd/<dirfd>/<stage>` so the formatter resolves the actual target hierarchy without inheriting the fd. Real Prettier coverage proves nearest `.prettierrc` discovery and explicit hidden-file formatting. This descriptor path is the narrow security-driven argv deviation from the approved canonical target. The stage contains the primitive's immediate bytes without an arbitrary size failure and is removed after success, expected failure, acquisition failure, defect, interruption, or Location shutdown. Cwd and Core discovery roots remain the active Location even for an approved external target. Child environment extends the inherited environment, then applies built-in and configured values without mutating `process.env`.
